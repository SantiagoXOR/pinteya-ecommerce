...
const ColorSwatch = ({
  ...
  <div className='flex items-center gap-3 p-3 border rounded-lg'>
    <div className='w-12 h-12 rounded-lg border shadow-sm' style={{ backgroundColor: value }} />
  ...
  <div className='space-y-3'>
    ...
    <div className='grid grid-cols-1 md:grid-cols-2 gap-2'>
      {Object.entries(scale).map(([key, value]) => (
        <ColorSwatch key={key} name={`${name.toLowerCase()}.${key}`} value={value} />
      ))}
    </div>
  </div>
)
...
export const StatusColors: StoryObj = {
  render: () => (
    <div className='space-y-8'>
      ...
      />
      <ColorScale name='Error' scale={colors.error} description='Errores y estados destructivos' />
      <ColorScale
        name='Info'
        scale={colors.info}
...
export const AllColors: StoryObj = {
  render: () => (
    <div className='space-y-8'>
      ...
      <ColorScale name='Accent' scale={colors.accent} description='Bright Sun - Color de acento' />
