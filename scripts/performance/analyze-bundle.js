...
function isHeavyDependency(name) {
  const heavyDeps = [
    'puppeteer', 'swiper', 'mercadopago', '@reduxjs/toolkit',
    'react-redux', 'date-fns', 'sharp', 'lucide-react'
  ];
...
function isEssentialDependency(name) {
  const essential = [
    ...
    'eslint', 'tailwindcss', 'postcss', 'autoprefixer'
  ];
...
function generateOptimizationSuggestions(content, componentName) {
  ...
  if (componentName.toLowerCase().includes('home') || componentName.toLowerCase().includes('page')) {
...
async function generateOptimizationPlan() {
  ...
  if (results.bundleSize && results.bundleSize.totalSize > 5 * 1024 * 1024) { // > 5MB
    optimizations.push({
      priority: 'high',
...
async function generateReport() {
  ...
  const report = {
    summary: {
      ...
      bundleSize: results.bundleSize?.totalSize ? `${(results.bundleSize.totalSize / 1024 / 1024).toFixed(2)} MB` : 'Error',
...
${results.heavyComponents?.map(comp => `
### ${comp.name}
