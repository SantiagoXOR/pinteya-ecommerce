...
class CIPerformanceChecker {
  ...
  checkBudgetViolation(budgetName, budget, metrics) {
    ...
    const isError = isScoreMetric
      ? value < adjustedBudget.threshold
      : value > adjustedBudget.threshold
    ...
    try {
      ...
      this.results.comparison = {
        ...
        firstLoadJS: {
          ...
          changePercent:
            ((current.firstLoadJS - baseline.firstLoadJS) / baseline.firstLoadJS) * 100,
        },
        score: {
  ...
  async generateReports() {
    ...
    try {
      ...
      await fs.writeFile(path.join(CONFIG.outputPath, 'ci-performance-report.md'), markdownReport)

      // Generar reporte de violaciones CSV
      if (this.results.violations.length > 0) {
        const csvReport = this.generateViolationsCSV()
        await fs.writeFile(path.join(CONFIG.outputPath, 'budget-violations.csv'), csvReport)
      }
