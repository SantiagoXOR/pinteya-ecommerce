...
async function runQualityTests() {
  ...
  try {
    ...
    results.unitTests = await runCommand(
...
function generateRecommendations(report) {
  ...
  if (!report.results.visualRegression?.success) {
    recommendations.push(
      '- 🎨 **Visual Regression**: Configurar Chromatic o revisar cambios visuales'
    )
  }

  if (report.summary.passed === report.summary.totalTests) {
    recommendations.push(
      '🎉 **¡Excelente!** Todos los tests de calidad están pasando correctamente'
    )
  }
