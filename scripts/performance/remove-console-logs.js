...
const config = {
  ...
  keepPatterns: [/console\.error\(/, /console\.warn\(/, /logger\./, /log\./, /Logger\./],
  // Archivos que requieren logging especial
  specialFiles: ['middleware.ts', 'logger.ts', 'monitoring.ts', 'analytics.ts'],
}
...
function generateReport() {
  ...
  if (results.logsRemoved > 0) {
    console.log(
      `🎉 ¡Limpieza completada! Se eliminaron ${results.logsRemoved} console.log de producción.`
    )
  } else {
