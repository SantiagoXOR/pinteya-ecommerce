...
const CHECKOUT_STEPS = [
  ...
  },
  {
    ...
    actions: async (page) => {
      ...
      const fields = [
        { selectors: ['input[name="firstName"]', 'input[name="first_name"]', 'input[placeholder*="nombre" i]'], value: 'Juan' },
        { selectors: ['input[name="lastName"]', 'input[name="last_name"]', 'input[placeholder*="apellido" i]'], value: 'Pérez' },
        { selectors: ['input[name="email"]', 'input[type="email"]'], value: 'juan.perez@example.com' },
        { selectors: ['input[name="phone"]', 'input[type="tel"]', 'input[placeholder*="teléfono" i]'], value: '+54 11 1234-5678' }
      ];
  ...
  },
  {
    ...
    actions: async (page) => {
      ...
      const allFields = [
        ...
        { selectors: ['input[name="address"]', 'input[name="street"]'], value: 'Av. Corrientes 1234' },
  ...
  },
  {
    ...
    actions: async (page) => {
      ...
      await page.goto(`${CONFIG.baseUrl}/checkout/success?payment_id=123456789&status=approved&external_reference=test-order-123`);
      await page.waitForLoadState('networkidle');
      await page.waitForTimeout(CONFIG.waitTime);
    },
    fullPage: true
  },
  {
    ...
    actions: async (page) => {
      ...
      await page.goto(`${CONFIG.baseUrl}/checkout/failure?payment_id=123456789&status=rejected&external_reference=test-order-123`);
      await page.waitForLoadState('networkidle');
      await page.waitForTimeout(CONFIG.waitTime);
    },
    fullPage: true
  },
  {
    ...
    actions: async (page) => {
      ...
      await page.goto(`${CONFIG.baseUrl}/checkout/pending?payment_id=123456789&status=pending&external_reference=test-order-123`);
      await page.waitForLoadState('networkidle');
      await page.waitForTimeout(CONFIG.waitTime);
    },
    fullPage: true
...
async function captureRealScreenshots(includeOptional = false) {
  ...
  const allSteps = includeOptional ? [...CHECKOUT_STEPS, ...OPTIONAL_CHECKOUT_STEPS] : CHECKOUT_STEPS;
  ...
  try {
    ...
    const metadata = {
      ...
      description: 'Screenshots reales del flujo completo de checkout incluyendo páginas de resultado',
      coverage: {
        basic_flow: allSteps.filter(s => !s.id.startsWith('bonus')).length,
        result_pages: allSteps.filter(s => s.id.includes('success') || s.id.includes('failure') || s.id.includes('pending')).length,
