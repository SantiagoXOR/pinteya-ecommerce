...
async function optimizeLogo() {
  try {
    ...
    console.log(
      `📐 Logo original: ${metadata.width}x${metadata.height}, ${metadata.format}, ${Math.round(metadata.size / 1024)}KB`
    )
    ...
    for (const [name, config] of Object.entries(OPTIMIZATIONS)) {
      ...
      let pipeline = originalImage.clone().resize(config.width, config.height, {
        fit: 'contain',
      ...
      await pipeline.clone().png({ quality: config.quality, compressionLevel: 9 }).toFile(pngPath)
