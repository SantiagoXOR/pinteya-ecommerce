...
const testSuites = {
  ...
  api: {
    name: 'Tests de APIs',
    patterns: [
      'src/__tests__/api/monitoring-apis.test.ts'
    ],
  ...
  integration: {
    ...
    patterns: [
      'src/__tests__/integration/monitoring-integration.test.ts'
    ],
  ...
  e2e: {
    ...
    patterns: [
      'src/__tests__/e2e/monitoring-dashboard.e2e.test.ts'
    ],
...
function verifyTestFiles() {
  ...
  log(`\n📊 Resumen:`, 'bright');
  logInfo(`Total de archivos de test: ${Object.values(testSuites).reduce((acc, suite) => acc + suite.patterns.length, 0)}`);
  logInfo(`Archivos encontrados: ${Object.values(testSuites).reduce((acc, suite) => acc + suite.patterns.length, 0) - missingFiles}`);
...
function generateCoverageReport() {
  ...
  const command = 'npx jest --coverage --coverageDirectory=coverage/fase3/complete --testPathPattern="(circuit-breaker|audit-trail|enterprise-metrics|alert-system|health-checks|monitoring)"';
