...
async function optimizeAllImages() {
  ...
  try {
    ...
    for (const imageInfo of imageFiles) {
      ...
      if (result.success) {
        ...
        optimizationLog.push({
          ...
          webpSavings: ((result.originalSize - result.webpSize) / result.originalSize * 100).toFixed(1),
          jpegSavings: ((result.originalSize - result.jpegSize) / result.originalSize * 100).toFixed(1)
        });
      } else {
        errorCount++;
        optimizationLog.push({
    ...
    if (successCount > 0) {
      ...
      const jpegSavings = ((totalOriginalSize - totalJpegSize) / totalOriginalSize * 100).toFixed(1);
