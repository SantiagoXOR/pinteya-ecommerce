...
async function debugOrdersAPI() {
  try {
    ...
    const response = await fetch(
      'http://localhost:3000/api/admin/orders?page=1&limit=20&sort_by=created_at&sort_order=desc'
    )
    ...
    if (data.data?.orders && Array.isArray(data.data.orders)) {
      ...
      if (firstOrder) {
        ...
        // Simular validación
        const hasId =
          firstOrder.id && (typeof firstOrder.id === 'string' || typeof firstOrder.id === 'number')
