...
class PurchasePipelineTest {
  ...
  async step3_SelectProduct() {
    ...
    await this.page.click('[data-testid="product-card"]:first-child, .product-card:first-child, .grid > div:first-child');
  ...
  async step7_FillCheckoutForm() {
    ...
    try {
      await this.page.fill('input[name="name"], input[placeholder*="nombre"]', TEST_DATA.customer.name);
    ...
    try {
      await this.page.fill('textarea[name="observations"], textarea[placeholder*="observaciones"]', TEST_DATA.observations);
  ...
  async step9_SubmitOrder() {
    ...
    if (!submitted) {
      await this.logStep('Botón finalizar compra encontrado pero no clickeable', false, 'Verificar implementación');
    } else {
      await this.page.waitForLoadState('networkidle');
  ...
  async run() {
    try {
      ...
      console.log('================================');
      console.log(`✅ Pasos exitosos: ${report.summary.successfulSteps}/${report.summary.totalSteps}`);
