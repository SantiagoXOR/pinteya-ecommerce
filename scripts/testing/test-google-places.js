...
async function testGooglePlaces() {
  ...
  try {
    ...
    if (geocodeResponse.data.results.length > 0) {
      ...
      // Calcular confianza
      const confidence = result.geometry.location_type === 'ROOFTOP' ? 0.95 :
                        result.geometry.location_type === 'RANGE_INTERPOLATED' ? 0.85 :
                        result.geometry.location_type === 'GEOMETRIC_CENTER' ? 0.75 : 0.65;
