...
async function testSecureProductsAPI() {
  ...
  try {
    const baseUrl =
      process.env.NODE_ENV === 'production' ? 'https://www.pinteya.com' : 'http://localhost:3000'
...
async function testMonitoringAPI() {
  ...
  try {
    ...
    const response = await authenticatedRequest(`${baseUrl}/api/admin/monitoring?timeframe=1h`)
...
async function testRateLimiting() {
  ...
  try {
    ...
    const promises = []
    for (let i = 0; i < 10; i++) {
      promises.push(authenticatedRequest(`${baseUrl}/api/admin/products-secure?page=1&limit=1`))
    }
