...
function validateCriticalFunctionalities() {
  ...
  criticalAPIs.forEach(apiPath => {
    ...
    if (fs.existsSync(fullPath)) {
      ...
      const usesMigration =
        content.includes('checkAdminPermissions') ||
...
function generateRegressionReport(results) {
  const { filesValid, testResults, buildResult, functionalityResults } = results
  ...
  console.log(
    `• Funcionalidades críticas: ${functionalityResults.filter(f => f.success).length}/${functionalityResults.length}`
  )
...
async function main() {
  try {
    ...
    if (report.overallSuccess) {
      ...
      console.log(
        `• ${report.stats.functionalitiesWorking}/${report.stats.totalFunctionalities} funcionalidades críticas operativas`
      )
