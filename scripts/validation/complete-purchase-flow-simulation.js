...
class PurchaseFlowSimulator {
  ...
  async createPaymentPreference() {
    ...
    try {
      ...
      this.logger.log('5. Creación de Preferencia de Pago', 'ERROR', {
  ...
  async simulatePaymentProcess() {
    ...
    try {
      ...
      const paymentResult = {
        ...
        transaction_amount: this.cart.reduce((sum, item) => sum + (item.price * item.quantity), 0) + TEST_SHIPPING.cost
      };
    ...
    try {
      const confirmation = {
        ...
        totals: {
          ...
          total: this.cart.reduce((sum, item) => sum + (item.price * item.quantity), 0) + TEST_SHIPPING.cost
        },
        estimatedDelivery: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
        trackingNumber: `TRK${Math.floor(Math.random() * 1000000).toString().padStart(8, '0')}`
      };
  ...
  async runCompleteFlow() {
    ...
    try {
      ...
      for (let i = 0; i < steps.length; i++) {
        ...
        if (i === 7) { // Webhook step needs payment result
          result = await step(paymentResult);
        } else {
          result = await step();
          if (i === 6) { // Payment step returns result
