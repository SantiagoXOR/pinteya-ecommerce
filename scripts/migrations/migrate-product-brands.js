...
const brandPatterns = [
  ...
  },
  {
    ...
    patterns: [
      /^(.+?)\s+sherwin\s+williams$/i,
      /^sherwin\s+williams\s+(.+)$/i
    ]
  }
];
  ...
  try {
    ...
    results.forEach(result => {
      if (result.status === 'success') {
        console.log(`✅ ID ${result.id}: "${result.original}" → Marca: "${result.brand}", Nombre: "${result.name}"`);
      } else {
        console.log(`❌ ID ${result.id}: "${result.original}" - Error: ${result.error || 'Desconocido'}`);
      }
    });
