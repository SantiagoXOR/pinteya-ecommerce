...
class SecurityPenetrationTester {
  ...
  async runSecurityTests() {
    ...
    for (const route of PROTECTED_ROUTES) {
      ...
      console.log(`${statusIcon} ${route.url} - Status: ${result.status} (${result.protected ? 'PROTEGIDO' : 'VULNERABLE'})`);
  ...
  generateSecurityReport() {
    ...
    // Guardar reporte
    const reportPath = path.join(__dirname, '..', 'test-results', 'security-penetration-report.json');
