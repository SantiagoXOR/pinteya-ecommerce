...
function validateWebhookFiles() {
  const webhookFiles = [
    ...
    'src/__tests__/webhook-robust.test.ts'
  ];
  ...
  const checks = [
    {
      ...
      test: content.includes('svix-id') && content.includes('svix-timestamp') && content.includes('svix-signature'),
      required: true
    },
    {
    ...
    },
    {
      ...
      test: content.includes('status: 400') && content.includes('status: 401') && content.includes('status: 500'),
      required: true
    }
  ];
