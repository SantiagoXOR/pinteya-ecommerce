...
function generateRecommendations(summary) {
  ...
  if (summary.averages.performance < 90) {
    recommendations.push('- 🎯 **Performance**: Optimizar imágenes, implementar lazy loading, reducir bundle size');
  }
  
  if (summary.averages.accessibility < 95) {
    recommendations.push('- ♿ **Accessibility**: Mejorar contraste de colores, agregar aria-labels, optimizar navegación por teclado');
  }
  
  if (summary.averages.fcp > 1800) {
    recommendations.push('- ⚡ **FCP**: Optimizar critical CSS, implementar preload de recursos importantes');
  }
  
  if (summary.averages.lcp > 2500) {
    recommendations.push('- 📊 **LCP**: Optimizar imágenes hero, implementar CDN, mejorar server response time');
  }
  
  if (summary.averages.cls > 0.1) {
    recommendations.push('- 📐 **CLS**: Definir dimensiones de imágenes, evitar inserción dinámica de contenido');
  }
