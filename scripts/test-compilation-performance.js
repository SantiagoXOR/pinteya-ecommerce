...
function generateReport() {
  ...
  results.summary = {
    ...
    averageTime: successfulTests.length > 0 
  ...
  if (buildTest && buildTest.success) {
    ...
    console.log(`Build Time: ${buildTest.duration}ms ${improvement ? '✅' : '❌'} (Target: ${TARGETS.buildTime}ms)`);
  }
  
  if (startupTest && startupTest.success) {
    const improvement = startupTest.duration < TARGETS.startupTime;
    console.log(`Startup Time: ${startupTest.duration}ms ${improvement ? '✅' : '❌'} (Target: ${TARGETS.startupTime}ms)`);
  }
