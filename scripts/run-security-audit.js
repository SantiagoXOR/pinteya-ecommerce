  ...
  async auditAuthenticationLogs() {
    ...
    try {
      ...
      this.results.modules.authLogs = {
        ...
        severity: analyzer.suspiciousPatterns.some(p => p.severity === 'critical') ? 'critical' :
                 analyzer.suspiciousPatterns.some(p => p.severity === 'high') ? 'high' : 'low',
        details: analyzer.suspiciousPatterns,
        stats: analyzer.stats
      };
      
      this.updateSummary('authLogs', this.results.modules.authLogs.severity);
      console.log(`   ✅ Completado - ${analyzer.suspiciousPatterns.length} patrones sospechosos detectados\n`);
  ...
  async auditCorsConfiguration() {
    ...
    try {
      ...
      const severity = findings.some(f => f.severity === 'high') ? 'high' : 
                      findings.some(f => f.severity === 'warning') ? 'warning' : 'low';
            ...
            Object.entries(audit.vulnerabilities).forEach(([name, vuln]) => {
              findings.push({
                ...
                severity: vuln.severity === 'critical' ? 'critical' : 
                         vuln.severity === 'high' ? 'high' : 'warning',
                package: name,
                details: vuln
              });
            });
