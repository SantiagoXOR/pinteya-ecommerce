 ...
 * 
 * Uso: npx ts-node scripts/validate-orders-integrity.ts
 */

const { createAdminClient } = require('../src/lib/integrations/supabase/server')

// Para hacer que TypeScript reconozca este archivo como módulo
export {}
...
class OrdersValidator {
  ...
  async validateOrders(limit: number = 50): Promise<ValidationResult> {
    ...
    try {
      ...
      for (const order of orders) {
        const validationData: OrderValidationData = {
          ...
          items_total: order.order_items?.reduce((sum: number, item: any) => sum + (item.price * item.quantity), 0) || null
        }
  ...
  private async validateSingleOrder(order: OrderValidationData): Promise<void> {
