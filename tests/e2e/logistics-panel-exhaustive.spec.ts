...
test.describe('Panel de Logística - Testing Exhaustivo', () => {
  ...
  test('Consistencia: Datos entre pestañas', async () => {
    await test.step('Verificar consistencia de datos entre vistas', async () => {
      ...
      const totalShipmentsText = await page.locator('text=Total Envíos').locator('..').locator('[class*="text-2xl"]').textContent();
  ...
  test('Database: Comparación con datos reales', async () => {
    await test.step('Comparar datos del UI con base de datos', async () => {
      ...
      const totalShipmentsUI = await page.locator('text=Total Envíos').locator('..').locator('[class*="text-2xl"]').textContent();
      ...
      const activeShipmentsUI = await page.locator('text=Envíos Activos').locator('..').locator('[class*="text-2xl"]').textContent();
...
async function generateTestReport(report: LogisticsTestReport) {
  ...
  report.summary.dataIntegrityScore = Math.round((report.summary.passedTests / report.summary.totalTests) * 100);
