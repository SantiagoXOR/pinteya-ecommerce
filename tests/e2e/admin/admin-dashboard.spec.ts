...
test.describe('Panel Administrativo - Dashboard', () => {
  ...
  test('debe cargar el dashboard administrativo correctamente', async ({ page }) => {
    ...
    const adminNavigation = ['Productos', 'Órdenes', 'Usuarios', 'Analytics', 'Configuración']

    for (const navItem of adminNavigation) {
      const element = page
        .locator(
          `text=${navItem}, [aria-label*="${navItem}"], [data-testid*="${navItem.toLowerCase()}"]`
        )
        .first()
  ...
  test('debe mantener autenticación durante la sesión', async ({ page }) => {
    ...
    const adminPages = ['/admin', '/admin/products', '/admin/orders', '/admin/users']
  ...
  test('debe mostrar información del usuario autenticado', async ({ page }) => {
    ...
    if (!userInfoFound) {
      console.log(
        '⚠️ No se encontró información específica del usuario, verificando contenido general'
      )
