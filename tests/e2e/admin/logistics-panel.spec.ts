...
test.describe('Panel de Logística Admin', () => {
  ...
  test('debe cargar el dashboard de logística correctamente', async ({ page }) => {
    ...
    await expect(page.getByText('Gestión completa de envíos y tracking en tiempo real')).toBeVisible();
  ...
  test('debe mostrar gráficos de performance', async ({ page }) => {
    ...
    const chartElements = page.locator('canvas, svg, .recharts-wrapper, [data-testid="performance-chart"]');
  ...
  test('debe abrir modal de crear envío', async ({ page }) => {
    ...
    if (count > 0) {
      ...
      const closeButton = page.locator('button:has-text("Cerrar"), button:has-text("Cancelar"), [aria-label="Close"]').first();
