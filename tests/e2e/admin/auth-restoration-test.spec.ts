...
test.describe('Authentication Restoration Tests', () => {
  ...
  test('should require authentication for /admin in production', async ({ page }) => {
    ...
    const isOnAdminPage =
      currentUrl.includes('/admin') && bodyText?.includes('Bienvenido al Panel Administrativo')

    const hasClerkAuth =
      bodyText?.includes('clerk') ||
    ...
    requests
      .filter(
        req => req.url.includes('/admin') || req.url.includes('/sign') || req.url.includes('clerk')
      )
      .forEach(req => {
  ...
  test('should test complete authentication flow', async ({ page }) => {
    ...
    const step1HasLogin =
      (await page.locator('text=Sign in, text=Iniciar sesión, [data-testid="sign-in"]').count()) > 0
    ...
    const step2HasLogin =
      (await page.locator('text=Sign in, text=Iniciar sesión, [data-testid="sign-in"]').count()) > 0
    const step2HasAdmin =
      (await page.locator('text=Gestión de Productos, text=Admin Panel').count()) > 0
