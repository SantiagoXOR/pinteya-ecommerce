...
test.describe('Production Admin Panel Tests', () => {
  ...
  test('should load https://pinteya.com/admin without errors', async ({ page }) => {
    ...
    const hasAdminLayout = await page
      .locator('[data-testid="admin-layout"], .admin-layout, main')
      .count()
    ...
    consoleErrors
      ...
      .forEach(log => console.log('⚠️', log))

    console.log('\n🔍 Requests importantes:')
    requests
      .filter(req => req.url.includes('/admin') || req.url.includes('/api/'))
      .forEach(req => {
  ...
  test('should compare /admin with /admin/products in production', async ({ page }) => {
    ...
    const productsIsClerkError =
      productsBodyText?.includes('redirect_url') && productsBodyText?.includes('invalid')
    ...
    const adminIsClerkError =
      adminBodyText?.includes('redirect_url') && adminBodyText?.includes('invalid')
