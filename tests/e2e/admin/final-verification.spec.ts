...
test.describe('Final Authentication Verification', () => {
  test('should verify final authentication state in production', async ({ page }) => {
    ...
    const hasLoginContent =
      adminContent?.includes('Sign in') || adminContent?.includes('Iniciar sesión')
    ...
    const hasProductsLogin =
      productsContent?.includes('Sign in') || productsContent?.includes('Iniciar sesión')
    ...
    const authFullyRestored =
      (adminRequiresAuth || productsRequiresAuth) && redirectWorks && siteWorks
