...
async function verifyCommonHeader(page: Page) {
  ...
  await expect(page.locator('[data-testid="cart-icon"], [data-testid="floating-cart-icon"]').first()).toBeVisible();
}
...
test.describe('Sistema de Filtros Avanzados - ConditionalContent', () => {
  ...
  test('Vista filtrada con search - debe mostrar FilteredProductsSection', async ({ page }) => {
    ...
    const hasProducts = await page.locator('article:has(img), [data-testid="product-card"]').count() > 0;
    ...
    const productCards = page.locator('[data-testid="product-card"], .product-card, article:has(img)');
...
test.describe('Sistema de Filtros Avanzados - FilteredProductsSection', () => {
  ...
  test('Grid de productos filtrados', async ({ page }) => {
    ...
    const productGrid = page.locator('[class*="grid"], [class*="products"], main > div:has(article)');
    await expect(productGrid.first()).toBeVisible();
    
    // Verificar que hay productos individuales
    const productCards = page.locator('article:has(img), [data-testid="product-card"], .product-card');
    const productCount = await productCards.count();
    expect(productCount).toBeGreaterThan(0);
    ...
    if (productCount > 0) {
      ...
      await expect(firstProduct.locator('button:has-text("Agregar"), button:has-text("carrito")')).toBeVisible();
    }
    
    console.log(`✅ Grid de productos filtrados test passed - ${productCount} productos encontrados`);
  });
