...
test.describe('Debug Categories API', () => {
  test('Debug - Test directo de API categorías', async ({ request }) => {
  ...
  test('Debug - Test API sin autenticación', async ({ request }) => {
    console.log('🔍 Probando API sin headers de autenticación...')
    
    const response = await request.get('/api/admin/categories')
    console.log(`📊 Status sin auth: ${response.status()}`)
    
    // Debería fallar con 401 o 403
