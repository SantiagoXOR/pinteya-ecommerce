...
test.describe('Flujo de Usuario - Verificación Datos Reales', () => {
  ...
  test('Flujo completo: Homepage → Búsqueda → Verificación datos dinámicos', async ({ page }) => {
    ...
    const searchInput = page.locator('input[type="search"], input[placeholder*="buscar"], input[placeholder*="Buscar"], #search-input, [data-testid="search-input"]').first();
    ...
    const hasHardcodedValues = trending.some((item: any) => 
