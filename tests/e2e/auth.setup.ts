...
setup('authenticate as admin', async ({ page }) => {
  ...
  try {
    ...
    fs.writeFileSync(`test-results/auth-setup-page-${Date.now()}.html`, html)
...
setup('verify authentication persistence', async ({ page }) => {
  ...
  try {
    ...
    if (!authFileExists) {
      console.log(
        '⚠️ Archivo de autenticación no encontrado después de esperar, creando configuración básica...'
      )
  ...
  try {
    ...
    if (fs.existsSync(authDir)) {
      ...
      const oldFiles = files.filter(
        file =>
          file.startsWith('admin-') && file.endsWith('.json') && file !== path.basename(authFile)
      )
