...
async function globalTeardown(config: FullConfig) {
  ...
  try {
    ...
    try {
      ...
      diagnosticFiles.screenshots = screenshotFiles.filter(f => 
        f.includes('admin-panel') && f.endsWith('.png')
      );
    } catch (error) {
    ...
    try {
      ...
      diagnosticFiles.reports = reportFiles.filter(f => 
        f.includes('admin-panel-diagnostic') && 
    ...
    if (diagnosticFiles.reports.length > 0) {
      ...
      const jsonReports = diagnosticFiles.reports.filter(f => f.endsWith('.json') && f !== 'diagnostic-metadata.json');
