...
test.describe('ProductCard - microinteracciones en hover', () => {
  test('muestra efectos de hover (transform o quick actions visibles)', async ({ page }, testInfo) => {
    ...
    if (!hasQuickActions) {
      const result = await expect
        .poll(async () => await card.evaluate((el) => getComputedStyle(el as HTMLElement).transform), {
          timeout: 1500,
      ...
      if (!hasTransformHover) {
        const transformAfter = await card.evaluate((el) => getComputedStyle(el as HTMLElement).transform);
        console.log('🔎 Transform luego de hover:', transformAfter);
        hasTransformHover = !!transformAfter && transformAfter !== 'none' && transformAfter !== transformBefore;
      }
    }
