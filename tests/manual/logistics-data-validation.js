...
function validateUIElements() {
  ...
  const checks = [
    ...
    { name: 'Badge de estado del sistema', selector: '[class*="badge"]', expectedText: 'Sistema' },
    {
      ...
      selector: 'button:has-text("Actualizar")',
      expectedText: 'Actualizar',
    },
    {
      name: 'Botón Crear Envío',
      selector: 'button:has-text("Crear Envío")',
      expectedText: 'Crear Envío',
    },
    ...
    { name: 'Pestaña Envíos', selector: '[data-value="shipments"]', expectedText: 'Envíos' },
    {
      ...
      selector: '[data-value="performance"]',
      expectedText: 'Performance',
    },
  ...
  const results = checks.map(check => {
    ...
    console.log(
      `${exists && hasCorrectText ? '✅' : '❌'} ${check.name}: ${exists ? 'Encontrado' : 'No encontrado'}`
    )
...
async function validateTabConsistency() {
  ...
  if (overviewTab) {
    ...
    const totalShipmentsElement = document
      .querySelector('text=Total Envíos')
      ?.parentElement?.querySelector('[class*="text-2xl"]')
    const totalFromOverview = totalShipmentsElement
      ? parseInt(totalShipmentsElement.textContent.replace(/[^\d]/g, ''))
      : 0
...
async function testInteractions() {
  ...
  if (refreshButton) {
    ...
    console.log(
      `${apiCalled ? '✅' : '❌'} Botón Actualizar: ${apiCalled ? 'Funciona' : 'No funciona'}`
    )
  }
  ...
  if (searchInput) {
    ...
    console.log(
      `${searchWorks ? '✅' : '❌'} Búsqueda: ${searchWorks ? 'Funciona' : 'No funciona'}`
    )
...
async function runCompleteValidation() {
  ...
  try {
    ...
    const formatsValid =
      report.results.formats.dates.every(d => d.isValid) &&
    ...
    const overallScore = [apiValid, uiValid, metricsValid, formatsValid, interactionsValid].filter(
      Boolean
    ).length
