...
function validateShipmentsAPI(data) {
  ...
  if (Array.isArray(data)) {
    ...
    if (data.length > 0) {
      ...
      const requiredFields = ['id', 'tracking_number', 'status', 'courier', 'destination', 'cost', 'created_at'];
...
async function runAPIValidation() {
  ...
  log('info', `Endpoints exitosos: ${report.summary.successfulEndpoints}/${report.summary.totalEndpoints}`);
