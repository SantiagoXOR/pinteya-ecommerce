      ...
      <div className="max-w-4xl mx-auto px-4">
        <div className="bg-white rounded-lg shadow-lg p-6">
          ...
          <div className="mb-8 p-4 bg-blue-50 rounded-lg">
            ...
            {user ? (
              <div className="space-y-2 text-sm">
                ...
                <p><strong>Rol (public):</strong> {(user.publicMetadata?.role as string) || 'No definido'}</p>
                <p><strong>Rol (private):</strong> {(user.privateMetadata?.role as string) || 'No definido'}</p>
          ...
          <div className="mb-8">
            ...
            {diagnostics && (
              <div className={`p-4 rounded-lg ${diagnostics.success ? 'bg-green-50' : 'bg-red-50'}`}>
                <div className="flex items-center mb-2">
                  <span className={`text-lg mr-2 ${diagnostics.success ? 'text-green-600' : 'text-red-600'}`}>
                    {diagnostics.success ? '✅' : '❌'}
                  </span>
                  <span className={`font-medium ${diagnostics.success ? 'text-green-800' : 'text-red-800'}`}>
                    {diagnostics.message}
                  </span>
                </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <ul className="mt-1 space-y-1">
                          <li>Supabase URL: {diagnostics.data.environment?.hasSupabaseUrl ? '✅' : '❌'}</li>
                          <li>Supabase Service Key: {diagnostics.data.environment?.hasSupabaseServiceKey ? '✅' : '❌'}</li>
                          <li>Clerk Secret: {diagnostics.data.environment?.hasClerkSecretKey ? '✅' : '❌'}</li>
                          <li>Clerk Publishable: {diagnostics.data.environment?.hasClerkPublishableKey ? '✅' : '❌'}</li>
                        </ul>
                      </div>
                      <div>
                        <h4 className="font-medium text-gray-700">Base de Datos:</h4>
                        <ul className="mt-1 space-y-1">
                          <li>Conexión: {diagnostics.data.supabase?.connection === 'ok' ? '✅' : '❌'}</li>
                          <li>Tabla products: {diagnostics.data.supabase?.productsTable === 'ok' ? '✅' : '❌'}</li>
                          <li>Tabla categories: {diagnostics.data.supabase?.categoriesTable === 'ok' ? '✅' : '❌'}</li>
                        </ul>
                      </div>
                    </div>
          ...
          <div className="mb-8">
            ...
            {apiTest && (
              <div className={`p-4 rounded-lg ${apiTest.success ? 'bg-green-50' : 'bg-red-50'}`}>
                <div className="flex items-center mb-2">
                  <span className={`text-lg mr-2 ${apiTest.success ? 'text-green-600' : 'text-red-600'}`}>
                    {apiTest.success ? '✅' : '❌'}
                  </span>
                  <span className={`font-medium ${apiTest.success ? 'text-green-800' : 'text-red-800'}`}>
                    {apiTest.message}
                  </span>
                </div>
