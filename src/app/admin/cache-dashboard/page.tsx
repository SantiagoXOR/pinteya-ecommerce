...
export default function CacheDashboard() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const [overview, strategies, layers, alerts, trends, topKeys, warmupTasks] =
        await Promise.all([
          fetch('/api/cache/overview')
            .then(r => r.json())
            .catch(() => mockOverview),
          fetch('/api/cache/strategies')
            .then(r => r.json())
            .catch(() => mockStrategies),
          fetch('/api/cache/layers')
            .then(r => r.json())
            .catch(() => mockLayers),
          fetch('/api/cache/alerts')
            .then(r => r.json())
            .catch(() => mockAlerts),
          fetch(`/api/cache/trends?period=${selectedPeriod}`)
            .then(r => r.json())
            .catch(() => mockTrends),
          fetch('/api/cache/top-keys')
            .then(r => r.json())
            .catch(() => mockTopKeys),
          fetch('/api/cache/warmup-tasks')
            .then(r => r.json())
            .catch(() => mockWarmupTasks),
        ])
    ...
    <div className='container mx-auto p-6 space-y-6'>
      ...
      <div className='flex justify-between items-center'>
        <div>
          ...
          <p className='text-muted-foreground'>Monitoreo y gestión del sistema de cache avanzado</p>
        </div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4'>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className='text-xs text-muted-foreground mt-2'>Promedio de todas las capas</p>
          </CardContent>
        </Card>
        ...
        <TabsContent value='layers'>
          <Card>
            <CardHeader>
              ...
              <CardDescription>Estado y performance de las capas multi-nivel</CardDescription>
            </CardHeader>
            <CardContent>
              <div className='space-y-4'>
              ...
              <CardContent>
                <div className='space-y-4'>
                  <div className='flex justify-between items-center'>
                    <span>Hit Rate</span>
                    <span
                      className={data.trends.hitRateTrend >= 0 ? 'text-green-600' : 'text-red-600'}
                    >
                      {data.trends.hitRateTrend >= 0 ? '+' : ''}
                      {data.trends.hitRateTrend.toFixed(1)}%
                    </span>
                  </div>
                  <div className='flex justify-between items-center'>
                    <span>Tiempo de Respuesta</span>
                    <span
                      className={data.trends.responseTrend <= 0 ? 'text-green-600' : 'text-red-600'}
                    >
                      {data.trends.responseTrend >= 0 ? '+' : ''}
                      {data.trends.responseTrend.toFixed(1)}%
                    </span>
                  </div>
                  <div className='flex justify-between items-center'>
                    <span>Volumen</span>
                    <span
                      className={data.trends.volumeTrend >= 0 ? 'text-blue-600' : 'text-gray-600'}
                    >
                      {data.trends.volumeTrend >= 0 ? '+' : ''}
                      {data.trends.volumeTrend.toFixed(1)}%
                    </span>
                  </div>
                  <div className='flex justify-between items-center'>
                    <span>Errores</span>
                    <span
                      className={data.trends.errorTrend <= 0 ? 'text-green-600' : 'text-red-600'}
                    >
                      {data.trends.errorTrend >= 0 ? '+' : ''}
                      {data.trends.errorTrend.toFixed(1)}%
                    </span>
                  </div>
                </div>
              </CardContent>
                ...
                <div className='space-y-3'>
                  {data.topKeys.slice(0, 8).map((key, index) => (
                    <div key={key.key} className='flex items-center justify-between'>
                      ...
                      <div className='text-right'>
                        <span className='text-sm font-medium'>{formatPercentage(key.hitRate)}</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <CardHeader>
              <div className='flex justify-between items-center'>
                <div>
                  ...
                  <CardDescription>Gestión de tareas de warmup de cache</CardDescription>
                </div>
...
const mockAlerts = [
  {
    ...
    message: 'Hit rate bajo en capa CDN',
  ...
  },
  {
    ...
    message: 'Memoria cache al 85% de capacidad',
...
const mockTopKeys = [
  ...
  { key: 'user:sessions', hits: 634, misses: 67, hitRate: 0.9 },
