        ...
        {debugInfo && (
          <>
            ...
            <div className='grid grid-cols-1 md:grid-cols-3 gap-6 mb-6'>
              <div
                className={`bg-white rounded-lg shadow-md p-6 ${debugInfo.checks.isPublicRoleAdmin ? 'border-l-4 border-green-500' : 'border-l-4 border-red-500'}`}
              >
                <h3 className='font-bold text-lg mb-2'>Public Metadata</h3>
                <p
                  className={`text-2xl font-bold ${debugInfo.checks.isPublicRoleAdmin ? 'text-green-600' : 'text-red-600'}`}
                >
              ...
              <div
                className={`bg-white rounded-lg shadow-md p-6 ${debugInfo.checks.isPrivateRoleAdmin ? 'border-l-4 border-green-500' : 'border-l-4 border-yellow-500'}`}
              >
                <h3 className='font-bold text-lg mb-2'>Private Metadata</h3>
                <p
                  className={`text-2xl font-bold ${debugInfo.checks.isPrivateRoleAdmin ? 'text-green-600' : 'text-yellow-600'}`}
                >
              ...
              <div className='bg-white rounded-lg shadow-md p-6 border-l-4 border-blue-500'>
                ...
                <p className='text-lg font-semibold text-blue-600'>{debugInfo.email}</p>
                <p className='text-sm text-gray-600'>ID: {debugInfo.userId}</p>
              </div>
            </div>
            ...
            <div className='bg-white rounded-lg shadow-md p-6 mb-6'>
              ...
              <div className='space-y-4'>
                <div
                  className={`p-4 rounded-lg ${debugInfo.checks.isPublicRoleAdmin ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}
                >
                  ...
                  <p>
                    ...
                    <span className='text-gray-500'>
                      {' '}
                      (tipo: {debugInfo.checks.publicRoleType})
                    </span>
                  </p>
                  <p>
                    <strong>Condición middleware:</strong> publicMetadata.role === 'admin' →{' '}
                    {debugInfo.checks.isPublicRoleAdmin ? 'true' : 'false'}
                  </p>
                  {!debugInfo.checks.isPublicRoleAdmin && (
                    <p className='text-red-600 mt-2'>
                      🔧 <strong>Problema:</strong> El middleware no detectará rol admin. Configurar
                      role: "admin" en public metadata.
                    </p>
                  )}
                </div>

                <div
                  className={`p-4 rounded-lg ${debugInfo.checks.isPrivateRoleAdmin ? 'bg-green-50 border border-green-200' : 'bg-yellow-50 border border-yellow-200'}`}
                >
                  <h3 className='font-bold mb-2'>
                  ...
                  <p>
                    ...
                    <span className='text-gray-500'>
                      {' '}
                      (tipo: {debugInfo.checks.privateRoleType})
                    </span>
                  </p>
                  <p>
                    <strong>Estado:</strong>{' '}
                    {debugInfo.checks.isPrivateRoleAdmin
