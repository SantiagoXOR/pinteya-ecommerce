  ...
  const runTests = async () => {
    ...
    try {
      ...
      testResults.push({
        ...
        message: sessionData?.user ? `Usuario: ${sessionData.user.email}` : 'No hay sesión activa',
        details: sessionData,
      })
    } catch (error) {
      testResults.push({
    ...
    try {
      const protectedRoutes = ['/api/admin/orders', '/api/admin/users', '/api/admin/analytics']
      ...
      for (const route of protectedRoutes) {
        ...
        testResults.push({
          ...
          message: response.ok ? 'Acceso permitido' : `Bloqueado (${response.status})`,
