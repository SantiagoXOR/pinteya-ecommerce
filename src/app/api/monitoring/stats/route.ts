...
async function getAlertStats(periodHours: number) {
  ...
  const alertsBySeverity = periodAlerts.reduce(
    (acc, alert) => {
  ...
  // Agrupar por tipo
  const alertsByType = periodAlerts.reduce(
    (acc, alert) => {
...
async function getBudgetStats(periodHours: number) {
  ...
  const budgetsByStatus = budgetReport.budgetResults.reduce(
    (acc, result) => {
  ...
  const budgetsByTrend = budgetReport.budgetResults.reduce(
    (acc, result) => {
...
async function getStreamingStats() {
  ...
  const clientsBySubscription = activeClients.reduce(
    (acc, client) => {
