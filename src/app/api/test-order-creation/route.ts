...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Calcular totales
    const itemsTotal = testOrderData.items.reduce(
    ...
    logs.push(
      `[TEST_ORDER] Totales calculados: items=${itemsTotal}, shipping=${shippingCost}, total=${totalAmount}`
    )
    ...
    if (orderError) {
      ...
      return NextResponse.json(
        { error: 'Order creation failed', logs, orderError },
        { status: 500 }
      )
    }
    ...
    if (updateError) {
      logs.push(
        '[TEST_ORDER] WARNING: Error actualizando preference_id: ' + JSON.stringify(updateError)
      )
    } else {
