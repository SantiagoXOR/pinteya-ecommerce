...
export async function GET(request: NextRequest) {
  try {
    ...
    if (driverError || !driver) {
      return NextResponse.json({ error: 'Driver no encontrado o no disponible' }, { status: 403 })
    }
    ...
    console.log(
      '📦 First order sample:',
      orders?.[0] ? JSON.stringify(orders[0], null, 2) : 'No orders'
    )
    ...
    const validOrders =
      orders?.filter(order => {
        ...
        return address && address.street_name && address.street_number && address.city_name
      }) || []
