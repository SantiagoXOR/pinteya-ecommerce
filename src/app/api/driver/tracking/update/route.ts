...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.driverId || !body.orderId || !body.currentLocation) {
      return NextResponse.json({ error: 'Datos de tracking incompletos' }, { status: 400 })
    }
    ...
    if (driverError || !driverData) {
      return NextResponse.json({ error: 'Driver no encontrado o no autorizado' }, { status: 403 })
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!driverId || !orderId) {
      return NextResponse.json({ error: 'Driver ID y Order ID requeridos' }, { status: 400 })
    }
