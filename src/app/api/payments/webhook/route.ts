...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!originValidation) {
      ...
      logger.security(
        LogLevel.ERROR,
        'Invalid webhook origin detected',
        {
      ...
      if (process.env.NODE_ENV === 'development') {
        ...
        console.log(
          '[WEBHOOK_DEBUG] Para habilitar modo debug, configura: MERCADOPAGO_WEBHOOK_DEBUG=true'
        )
      }
...
async function processWebhookAsync(webhookData: MercadoPagoWebhookData, clientIP: string) {
  try {
    ...
    const webhookResult = await executeWebhookProcessing(async () => {
      ...
      if (
        webhookData.data.id === '123456' ||
        webhookData.data.id === 'test' ||
        webhookData.data.id === '106'
      ) {
      ...
      if (
        webhookData.data.id === 'test_payment_107' ||
        webhookData.data.id.toString().includes('test_payment_')
      ) {
      ...
      if (
        webhookData.data.id === 'manual_payment_108' ||
        webhookData.data.id === 'manual_test_payment'
      ) {
      ...
      if (
        webhookData.data.id === 'manual_payment_109' ||
        webhookData.data.id === 'new_manual_payment'
      ) {
    ...
    if (!webhookResult.success) {
      if (webhookResult.wasRejected) {
        logger.warn(
          LogLevel.WARN,
          'Webhook processing rejected by circuit breaker',
          {
            dataId: webhookData.data.id,
    ...
    console.log(
      '[WEBHOOK_ASYNC] 🔍 Payer info de MercadoPago:',
      JSON.stringify(payment.payer, null, 2)
    )
    console.log(
      '[WEBHOOK_ASYNC] 🔍 Additional info de MercadoPago:',
      JSON.stringify(payment.additional_info, null, 2)
    )
    console.log(
      '[WEBHOOK_ASYNC] 🔍 Shipments info:',
    ...
    logger.webhook(
      LogLevel.INFO,
      'Webhook processed successfully (async)',
      {
        type: 'payment',
        action: 'processed',
