...
async function processWebhookTestAsync(webhookData: MercadoPagoWebhookData, clientIP: string) {
  try {
    ...
    console.log(
      '[WEBHOOK_TEST_ASYNC] Actualizando orden con status:',
      newOrderStatus,
      'payment_status:',
      newPaymentStatus
    )
    ...
    console.log(
      '[WEBHOOK_TEST_ASYNC] 🔍 Updated payer info:',
      JSON.stringify(updatedPayerInfo, null, 2)
    )
    console.log(
      '[WEBHOOK_TEST_ASYNC] 🔍 Updated shipping address:',
      JSON.stringify(updatedShippingAddress, null, 2)
    )
