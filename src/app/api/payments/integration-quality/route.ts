...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    logger.performance(
      LogLevel.ERROR,
      'Integration quality measurement failed',
      {
...
async function checkSecurity(
  ...
  checks.push({
    ...
    description: hasSecureCredentials
...
async function checkUserExperience(supabase: SupabaseClient): Promise<QualityCheck> {
  ...
  checks.push({
    ...
    description: hasAutoReturn
      ? 'Auto return configurado'
      : 'Auto return deshabilitado en desarrollo',
 ...
 */
async function checkIntegrationCompleteness(
  supabase: SupabaseClient,
  paymentId?: string | null
): Promise<QualityCheck> {
  const checks = []
