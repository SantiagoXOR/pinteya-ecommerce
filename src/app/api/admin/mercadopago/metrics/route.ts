...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const totalSuccess =
      metrics.payment_creation.requests.success +
    ...
    const response: MetricsResponse = {
      ...
      data: {
        realTimeMetrics: {
          ...
          rateLimitHits:
            metrics.payment_creation.requests.rate_limited +
            metrics.payment_queries.requests.rate_limited +
            metrics.webhook_processing.requests.rate_limited,
          retryAttempts:
            metrics.payment_creation.retry_stats.total_retries +
        ...
        endpointMetrics: {
          createPreference: {
            ...
            successRate:
              metrics.payment_creation.requests.total > 0
                ? (metrics.payment_creation.requests.success /
                    metrics.payment_creation.requests.total) *
                  100
                : 100,
          ...
          webhook: {
            ...
            successRate:
              metrics.webhook_processing.requests.total > 0
                ? (metrics.webhook_processing.requests.success /
                    metrics.webhook_processing.requests.total) *
                  100
                : 100,
          ...
          paymentQuery: {
            ...
            successRate:
              metrics.payment_queries.requests.total > 0
                ? (metrics.payment_queries.requests.success /
                    metrics.payment_queries.requests.total) *
                  100
                : 100,
