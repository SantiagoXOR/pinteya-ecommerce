...
async function getDashboardMetrics(period: string, timezone: string): Promise<DashboardMetrics> {
  try {
    ...
    const previousRevenue =
      previousOrdersData?.reduce((sum, order) => sum + (order.total || 0), 0) || 0
    const revenueGrowth =
    ...
    const newCustomers =
      customersData?.filter(c => new Date(c.created_at) >= startDate)?.length || 0
    ...
    const metrics: DashboardMetrics = {
      ...
      inventory: {
        total_value:
          productsData?.reduce((sum, p) => sum + (p.price || 0) * (p.stock_quantity || 0), 0) || 0,
        low_stock_alerts: lowStockProducts,
        categories_count: categoriesData?.length || 0,
        average_stock_level:
          totalProducts > 0
            ? (productsData?.reduce((sum, p) => sum + (p.stock_quantity || 0), 0) || 0) /
              totalProducts
            : 0,
      },
      performance: {
