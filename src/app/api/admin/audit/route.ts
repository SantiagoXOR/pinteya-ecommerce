...
async function getAuditLogs(filters: z.infer<typeof AuditFiltersSchema>) {
  ...
  if (filters.search) {
    query = query.or(
      `table_name.ilike.%${filters.search}%,action.ilike.%${filters.search}%,record_id.ilike.%${filters.search}%`
    )
  }
...
async function getAuditStats(tenantId?: string): Promise<AuditStats> {
  ...
  // Resumen de acciones
  const actionsSummary = logs.reduce(
    (acc, log) => {
  ...
  // Resumen de tablas
  const tablesSummary = logs.reduce(
    (acc, log) => {
  ...
  // Resumen de usuarios
  const usersSummary = logs.reduce(
    (acc, log) => {
