...
import { validateData, sanitizeData } from '@/lib/validation/enterprise-validation-middleware'
import {
  criticalValidator,
  highValidator,
  standardValidator,
...
const postHandler = async (request: ValidatedRequest) => {
  try {
    ...
    const testData = {
      ...
      description:
        validatedData.description || 'Test description with <script>alert("xss")</script>',
    ...
    const demoStats = {
      ...
      security_patterns_detected: [
        ...(criticalResult.errors?.filter(
          e => e.code.includes('INJECTION') || e.code.includes('XSS')
        ) || []),
        ...(standardResult.errors?.filter(
          e => e.code.includes('INJECTION') || e.code.includes('XSS')
        ) || []),
      ].length,
      performance_metrics: {
        total_time_ms:
          (criticalResult.metadata?.performanceMs || 0) +
          (standardResult.metadata?.performanceMs || 0),
        average_time_ms:
          ((criticalResult.metadata?.performanceMs || 0) +
            (standardResult.metadata?.performanceMs || 0)) /
          2,
  ...
  try {
    ...
    securityTest.results = {
      ...
      security_patterns_found:
        testResult.errors?.filter(e => e.code.includes('INJECTION') || e.code.includes('XSS')) ||
        [],
