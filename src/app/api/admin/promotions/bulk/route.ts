...
const BulkPromotionActionSchema = z.object({
  ...
  promotion_ids: z
    .array(z.string().uuid())
    .min(1, 'Debe seleccionar al menos una promoción')
    .max(50, 'Máximo 50 promociones por operación'),
      ...
      switch (action) {
        ...
        case 'duplicate':
          ...
          const duplicateData = {
            ...
            coupon_code: fullPromotion.coupon_code
              ? `${fullPromotion.coupon_code}_COPY_${Date.now()}`
              : undefined,
            is_active: false,
            usage_count: 0,
            created_by: userId,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    logger.log(LogLevel.ERROR, LogCategory.API, 'Error en POST /api/admin/promotions/bulk', {
      error,
    })
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    logger.log(LogLevel.ERROR, LogCategory.API, 'Error en GET /api/admin/promotions/bulk', {
      error,
    })
