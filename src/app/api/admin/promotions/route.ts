...
const PromotionFiltersSchema = z.object({
  ...
  sort_by: z
    .enum(['created_at', 'name', 'priority', 'starts_at', 'ends_at', 'usage_count'])
    .default('created_at'),
...
async function getPromotions(filters: z.infer<typeof PromotionFiltersSchema>) {
  ...
  if (filters.status) {
    ...
    switch (filters.status) {
      ...
      case 'scheduled':
        ...
        break
      case 'expired':
        query = query.eq('is_active', true).not('ends_at', 'is', null).lt('ends_at', now)
        break
      case 'paused':
  ...
  if (filters.search) {
    query = query.or(
      `name.ilike.%${filters.search}%,description.ilike.%${filters.search}%,banner_text.ilike.%${filters.search}%`
    )
  }
...
async function createPromotion(
  ...
  switch (promotionData.type) {
    ...
    case 'buy_x_get_y':
      if (!promotionData.buy_quantity || !promotionData.get_quantity) {
        throw new Error(
          'Las cantidades de compra y obsequio son requeridas para este tipo de promoción'
        )
      }
      break
    case 'bundle_deal':
      if (!promotionData.bundle_products?.length || !promotionData.bundle_price) {
        throw new Error(
          'Los productos del bundle y el precio son requeridos para este tipo de promoción'
        )
      }
      break
  }
  ...
  if (promotionData.applicable_to === 'products' && product_ids?.length) {
    ...
    const { error: productError } = await supabase.from('promotion_products').insert(productInserts)
...
async function getPromotionStats(): Promise<PromotionStats> {
  ...
  const performanceMetrics = {
    last_24h: {
      ...
      discount_given: (usage || [])
        .filter(u => new Date(u.created_at) >= last24h)
        .reduce((sum, u) => sum + (u.discount_amount || 0), 0),
      orders_affected: new Set(
        (usage || []).filter(u => new Date(u.created_at) >= last24h).map(u => u.order_id)
      ).size,
    },
    last_7d: {
      usage: (usage || []).filter(u => new Date(u.created_at) >= last7d).length,
      discount_given: (usage || [])
        .filter(u => new Date(u.created_at) >= last7d)
        .reduce((sum, u) => sum + (u.discount_amount || 0), 0),
      orders_affected: new Set(
        (usage || []).filter(u => new Date(u.created_at) >= last7d).map(u => u.order_id)
      ).size,
    },
    last_30d: {
      usage: (usage || []).filter(u => new Date(u.created_at) >= last30d).length,
      discount_given: (usage || [])
        .filter(u => new Date(u.created_at) >= last30d)
        .reduce((sum, u) => sum + (u.discount_amount || 0), 0),
      orders_affected: new Set(
        (usage || []).filter(u => new Date(u.created_at) >= last30d).map(u => u.order_id)
      ).size,
    },
  }

  // Top promociones
  const promotionUsageMap = (usage || []).reduce(
    (acc, u) => {
  ...
  // Uso por tipo
  const usageByType = (promotions || []).reduce(
    (acc, promotion) => {
