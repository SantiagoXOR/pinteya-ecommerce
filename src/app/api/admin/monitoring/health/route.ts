...
export async function GET(request: NextRequest) {
  try {
    ...
    const statusCode =
      overall === HealthStatus.HEALTHY ? 200 : overall === HealthStatus.DEGRADED ? 200 : 503
  ...
  try {
    ...
    switch (action) {
      ...
      case 'recover':
        ...
        if (recoveryActionId) {
          const success = await enterpriseHealthSystem.executeRecoveryAction(
            recoveryActionId,
            config
          )
      ...
      case 'reset':
        if (service === 'circuit_breakers' || !service) {
          const success =
            await enterpriseHealthSystem.executeRecoveryAction('reset_circuit_breakers')
    ...
    logger.info(
      LogLevel.INFO,
      `Health action performed: ${action}`,
      {
