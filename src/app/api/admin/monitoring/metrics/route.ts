  ...
  return {
    ...
    throughput:
      throughputMetrics.length > 0
        ...
        : 120,
    uptime: uptimeMetrics.length > 0 ? uptimeMetrics[0].value : 0.9997,
  }
}
...
async function getInfrastructureMetrics() {
  ...
  if ([criticalBreakerState, standardBreakerState, webhookBreakerState].includes('open')) {
    ...
    [criticalBreakerState, standardBreakerState, webhookBreakerState].includes('half-open')
  ) {
