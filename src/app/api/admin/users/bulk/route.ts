...
const BulkUserOperationSchema = z.object({
  ...
  user_ids: z
    ...
    .max(MAX_BULK_OPERATIONS, `Máximo ${MAX_BULK_OPERATIONS} usuarios por operación`),
  data: z
    .object({
...
async function bulkDeleteUsers(
  ...
  const deletableUserIds = userIds.filter(id => id !== adminUserId && !userIdsWithOrders.has(id))
    ...
    const exportParams = {
      filters: {
        ...
        has_orders: searchParams.get('has_orders')
          ? searchParams.get('has_orders') === 'true'
          : undefined,
        min_orders: searchParams.get('min_orders')
          ? parseInt(searchParams.get('min_orders')!)
          : undefined,
        min_spent: searchParams.get('min_spent')
          ? parseFloat(searchParams.get('min_spent')!)
          : undefined,
      },
