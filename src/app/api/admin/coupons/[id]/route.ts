...
async function updateCoupon(
  ...
  if (updateData.starts_at || updateData.expires_at) {
    const startsAt = updateData.starts_at
      ? new Date(updateData.starts_at)
      : new Date(existingCoupon.starts_at)
    const expiresAt = updateData.expires_at
...
async function deleteCoupon(couponId: string, userId: string) {
  ...
  if (usageCount && usageCount > 0) {
    throw new Error(
      'No se puede eliminar un cupón que ya ha sido utilizado. Considere desactivarlo en su lugar.'
    )
  }
