...
async function validateOrderAccess(orderId: string, userId: string): Promise<any> {
  ...
  if (!['confirmed', 'processing'].includes(order.status)) {
    throw new Error(
      `La orden debe estar confirmada o en procesamiento para crear envío. Estado actual: ${order.status}`
    )
  }
...
async function createShipmentItems(
  ...
  const shipmentItems = requestedItems.map(reqItem => {
    ...
    if (reqItem.quantity > orderItem.quantity) {
      throw new Error(
        `Cantidad solicitada (${reqItem.quantity}) excede cantidad disponible (${orderItem.quantity})`
      )
    }
