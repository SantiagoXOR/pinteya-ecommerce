...
export async function POST(request: NextRequest) {
  try {
    ...
    if (automaticActions.length > 0) {
      await supabaseAdmin.from('incident_response_actions').insert(automaticActions)
    }
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const { incidentId, status, assignedTo, resolution, impactAssessment, addTimelineEntry } = body
    ...
    if (assignedTo !== undefined && assignedTo !== currentIncident.assigned_to) {
      ...
      timelineEntries.push({
        ...
        actor: context.userId,
        description: assignedTo ? `Incidente asignado a ${assignedTo}` : 'Asignación removida',
...
function generateAutomaticActions(
