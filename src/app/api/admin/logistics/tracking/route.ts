...
const BulkTrackingUpdateSchema = z.object({
  events: z
    .array(TrackingEventCreateSchema)
    .min(1, 'Debe incluir al menos un evento')
    .max(50, 'Máximo 50 eventos por lote'),
})
...
async function getHandler(
  ...
  const filters = TrackingFiltersSchema.parse({
    ...
    shipment_id: searchParams.get('shipment_id')
      ? parseInt(searchParams.get('shipment_id')!)
      : undefined,
    ...
    carrier_id: searchParams.get('carrier_id')
      ? parseInt(searchParams.get('carrier_id')!)
      : undefined,
