  ...
  try {
    ...
    if (driver_id) {
      ...
      if (driver.status !== 'available') {
        return NextResponse.json({ error: 'El driver no está disponible' }, { status: 400 })
      }
      ...
      if (updateError) {
        ...
        return NextResponse.json({ error: 'Error al desasignar driver' }, { status: 500 })
      }
...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (driversError) {
      ...
      return NextResponse.json({ error: 'Error al obtener drivers disponibles' }, { status: 500 })
    }
