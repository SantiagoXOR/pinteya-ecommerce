...
function calculateShippingCost(courier: Courier, weightKg: number, declaredValue?: number): number {
  let cost = courier.base_cost + courier.cost_per_kg * weightKg

  // Aplicar envío gratis si aplica
  if (
    courier.free_shipping_threshold &&
    declaredValue &&
    declaredValue >= courier.free_shipping_threshold
  ) {
    cost = 0
  }
...
export const POST = withTenantAdmin(async (
  ...
  try {
    ...
    if (existingCourier) {
      return NextResponse.json({ error: 'Courier code already exists' }, { status: 409 })
    }
...
export async function POST_QUOTE(request: NextRequest) {
  try {
    ...
    let query = supabase.from('couriers').select('*').eq('is_active', true)
      ...
      for (const service of servicesToQuote) {
        ...
        quotes.push({
          ...
          includes_insurance: validatedData.declared_value
            ? validatedData.declared_value <= 10000
            : false,
