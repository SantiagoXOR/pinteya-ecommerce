...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!url && !urls) {
      return NextResponse.json({
        ...
        data: {
          ...
          description:
            'Validate SEO metadata including titles, descriptions, keywords, and Open Graph tags',
          usage: {
    ...
    const resultsByUrl = testUrls.reduce(
      (acc, testUrl) => {
...
export async function POST(request: NextRequest) {
  try {
    ...
    logger.info(
      LogLevel.INFO,
      'Batch metadata validation completed',
      {
...
function generateMetadataRecommendations(
  ...
  const failedDescriptions = results.filter(
    r => r.testName.includes('Description') && r.status === 'failed'
  )
  const failedKeywords = results.filter(
    r => r.testName.includes('Keywords') && r.status !== 'passed'
  )
  const failedOpenGraph = results.filter(
    r => r.testName.includes('Open Graph') && r.status !== 'passed'
  )

  if (failedTitles.length > 0) {
    recommendations.push(
      `Fix ${failedTitles.length} title tag issues - ensure 30-60 character length`
    )
  }

  if (failedDescriptions.length > 0) {
    recommendations.push(
      `Improve ${failedDescriptions.length} meta descriptions - aim for 120-160 characters`
    )
  }

  if (failedKeywords.length > 0) {
    recommendations.push(
      `Optimize keywords for ${failedKeywords.length} pages - include 3-5 relevant terms`
    )
  }

  if (failedOpenGraph.length > 0) {
    recommendations.push(
      `Add Open Graph tags to ${failedOpenGraph.length} pages for better social sharing`
    )
  }
...
function analyzeMetadataResults(
  results: Array<{
    testName: string
    status: 'passed' | 'failed' | 'warning'
    score: number
  ...
  const titleIssues = results.filter(
    r => r.testName.includes('Title') && r.status !== 'passed'
  ).length

  const descriptionIssues = results.filter(
    r => r.testName.includes('Description') && r.status !== 'passed'
  ).length

  const keywordIssues = results.filter(
    r => r.testName.includes('Keywords') && r.status !== 'passed'
  ).length

  const openGraphIssues = results.filter(
    r => r.testName.includes('Open Graph') && r.status !== 'passed'
  ).length
  ...
  const urlsWithIssues = [...new Set(results.filter(r => r.status !== 'passed').map(r => r.url))]
...
function generateDetailedMetadataRecommendations(
  ...
  if (analysis.titleIssues > 0) {
    ...
    recommendations.push({
      ...
      solution:
        'Optimize title tags to 30-60 characters, include primary keywords, and ensure uniqueness',
      affectedUrls,
    })
  }
  ...
  if (analysis.descriptionIssues > 0) {
    ...
    recommendations.push({
      ...
      solution:
        'Write compelling meta descriptions of 120-160 characters with clear call-to-action',
      affectedUrls,
    })
  }
  ...
  if (analysis.openGraphIssues > 0) {
    ...
    recommendations.push({
      ...
      solution:
        'Add og:title, og:description, og:image, and og:url tags for better social media sharing',
      affectedUrls,
    })
  }
