...
export async function GET(request: NextRequest) {
  try {
    ...
    switch (action) {
      ...
      case 'history':
        const history = enhancedSEOTestingSuite.getTestHistory(limit ? parseInt(limit) : undefined)
        return NextResponse.json({
          success: true,
          data: history,
        })
  ...
  try {
    ...
    switch (action) {
      ...
      case 'validate-structured-data':
        const structuredDataResults = await enhancedSEOTestingSuite.runTestsByType(
          'structured_data',
          urls
        )
        result = {
...
function generateStructuredDataRecommendations(
  results: Array<{
    testName: string
    status: 'passed' | 'failed' | 'warning'
    message?: string
  ...
  const failedPresence = results.filter(
    r => r.testName.includes('Presence') && r.status === 'failed'
  )
  const failedValidation = results.filter(
    r => r.testName.includes('Validation') && r.status === 'failed'
  )
  ...
  if (failedValidation.length > 0) {
    recommendations.push(
      `Fix structured data validation errors on ${failedValidation.length} pages`
    )
  }
...
function generateLinksRecommendations(
  results: Array<{
    testName: string
    status: 'passed' | 'failed' | 'warning'
    message?: string
  ...
  const hierarchyIssues = results.filter(
    r => r.testName.includes('Hierarchy') && r.status !== 'passed'
  )
...
function generateComplianceRecommendations(
  ...
  const accessibilityIssues = results.filter(
    r => r.testName.includes('Accessibility') && r.status !== 'passed'
  )
