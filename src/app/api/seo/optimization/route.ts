    ...
    switch (action) {
      ...
      case 'recommendations':
        const recommendations =
          await enhancedSEOOptimizationTools.generateAutomatedRecommendations()
        return NextResponse.json({
          success: true,
  ...
  try {
    ...
    switch (action) {
      ...
      case 'optimize_content':
        ...
        result = await enhancedSEOOptimizationTools.optimizeContent(params.url, params.contentType)
        break
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    logger.error(
      LogLevel.ERROR,
      'SEO optimization API DELETE error',
      error as Error,
      LogCategory.SEO
    )
