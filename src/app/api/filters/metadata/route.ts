...
export async function GET(request: NextRequest) {
  try {
    ...
    if (brandsError) {
      ...
      return NextResponse.json({ success: false, error: 'Error fetching brands' }, { status: 500 })
    }
    ...
    if (!productNamesError && productNames) {
      ...
      paintTypes = typeKeywords.filter(type =>
        productNames.some(product => product.name.toLowerCase().includes(type.toLowerCase()))
      )

      finishes = finishKeywords.filter(finish =>
        productNames.some(product => product.name.toLowerCase().includes(finish.toLowerCase()))
      )
    }
