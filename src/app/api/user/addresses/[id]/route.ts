...
export async function GET(
  ...
  try {
    ...
    if (!supabaseAdmin) {
      console.error(
        'Cliente administrativo de Supabase no disponible en GET /api/user/addresses/[id]'
      )
...
export async function PUT(
  ...
  try {
    ...
    if (!supabaseAdmin) {
      console.error(
        'Cliente administrativo de Supabase no disponible en PUT /api/user/addresses/[id]'
      )
      return NextResponse.json(
        { error: 'Servicio de base de datos no disponible' },
        { status: 503 }
...
export async function DELETE(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!supabaseAdmin) {
      console.error(
        'Cliente administrativo de Supabase no disponible en DELETE /api/user/addresses/[id]'
      )
      return NextResponse.json(
        { error: 'Servicio de base de datos no disponible' },
        { status: 503 }
    ...
    const { error } = await supabaseAdmin.from('user_addresses').delete().eq('id', addressId)
