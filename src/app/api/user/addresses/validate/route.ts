...
async function validateAddressWithGooglePlaces(address: {
  ...
  try {
    ...
    console.log(
      `✅ Google Places: Dirección ${isValid ? 'válida' : 'inválida'} con ${Math.round(confidence * 100)}% confianza`
    )
...
async function enhancedSimulationValidation(address: {
  ...
  const argentinaCities: Record<string, { lat: number; lng: number; confidence_bonus: number }> = {
    ...
    'buenos aires': { lat: -34.6118, lng: -58.396, confidence_bonus: 0.1 },
  ...
  console.log(
    `✅ Validación mejorada: Dirección ${isValid ? 'válida' : 'inválida'} con ${Math.round(confidence * 100)}% confianza`
  )
...
async function fallbackValidation(address: {
  ...
  const cityKey = address.city.toLowerCase()
  const coordinates = cityCoordinates[cityKey] || { lat: -34.6118, lng: -58.396 } // Default Buenos Aires
