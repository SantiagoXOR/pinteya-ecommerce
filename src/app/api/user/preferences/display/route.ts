...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (!display) {
      return NextResponse.json({ error: 'Configuración de display requerida' }, { status: 400 })
    }

    // Validar configuraciones de display
    const validatedDisplay = validateDisplaySettings(display)
    if (!validatedDisplay) {
      return NextResponse.json({ error: 'Configuración de display inválida' }, { status: 400 })
    }
