...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!['json', 'csv'].includes(format)) {
      return NextResponse.json({ error: 'Formato no soportado. Use json o csv' }, { status: 400 })
    }
    ...
    if (Array.isArray(data)) {
      data.forEach((item: any, index: number) => {
        Object.entries(item).forEach(([key, value]) => {
          ...
          csvRows.push(
            `"${section}[${index}]","${key}","${csvValue.replace(/"/g, '""')}","${item.created_at || ''}"`
          )
        })
      })
      ...
      Object.entries(data).forEach(([key, value]) => {
        const csvValue = typeof value === 'object' ? JSON.stringify(value) : String(value || '')
        csvRows.push(
          `"${section}","${key}","${csvValue.replace(/"/g, '""')}","${data.created_at || data.updated_at || ''}"`
        )
      })
    }
  })
