...
export class PerformanceBudgetMonitor {
  ...
  private recordMetric(type: keyof PerformanceBudget, value: number) {
    ...
    let existingMetrics = this.metrics.find(
      m => m.url === currentUrl && timestamp - m.timestamp < 5000 // Agrupar métricas de los últimos 5 segundos
    )
  ...
  getCurrentMetrics(): PerformanceMetrics | null {
    const currentUrl = window.location.pathname
    return (
      this.metrics.filter(m => m.url === currentUrl).sort((a, b) => b.timestamp - a.timestamp)[0] ||
      null
    )
  }
