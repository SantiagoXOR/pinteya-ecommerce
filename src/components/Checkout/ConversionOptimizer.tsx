    ...
    <Card className={`bg-gradient-to-r ${getVariantStyles()} ${isUrgent ? 'animate-pulse' : ''}`}>
      <CardContent className='p-4'>
        <div className='flex items-center gap-3'>
          ...
          >
            <Clock
              className={`w-5 h-5 ${isUrgent ? 'text-red-600' : 'text-yellow-600'} ${
  ...
  return (
    <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3'>
      {trustItems.map((item, index) => (
        <Card
          key={index}
          className={`bg-${item.color}-50 border-${item.color}-200 hover:shadow-md transition-shadow`}
        >
      ...
      <Card className='max-w-md w-full bg-gradient-to-r from-red-50 to-orange-50 border-red-200'>
        <CardContent className='p-6 text-center'>
          ...
          <div className='flex gap-3'>
            <Button onClick={onAccept} className='flex-1 bg-red-600 hover:bg-red-700 text-white'>
              ¡Quiero mi descuento!
            </Button>
