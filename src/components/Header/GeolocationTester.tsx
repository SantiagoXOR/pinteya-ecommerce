...
const GeolocationTester = () => {
  ...
  return (
    <div className='max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-lg'>
      <h2 className='text-2xl font-bold mb-6 text-gray-800'>🧪 Geolocation Tester</h2>
      ...
      <div className={`p-4 rounded-lg mb-6 ${getStatusColor()}`}>
        ...
        <div className='space-y-1 text-sm'>
          <p>
            <strong>Permisos:</strong> {permissionStatus}
          </p>
          <p>
          ...
          <p>
            ...
            {location ? `${location.lat}, ${location.lng}` : 'No detectada'}
          </p>
          <p>
            <strong>Zona detectada:</strong> {detectedZone?.name || 'Ninguna'}
          </p>
        </div>
      </div>
      ...
      <div className='mt-6 p-4 bg-gray-50 rounded-lg'>
        ...
        <div className='text-sm space-y-1 text-gray-600'>
          <p>
            ...
            {'geolocation' in navigator ? '✅ Soportado' : '❌ No soportado'}
          </p>
          <p>
            <strong>Permissions API:</strong>{' '}
            {'permissions' in navigator ? '✅ Soportado' : '❌ No soportado'}
          </p>
          <p>
            <strong>HTTPS:</strong>{' '}
            {typeof window !== 'undefined' && window.location?.protocol === 'https:'
              ? '✅ Seguro'
              : '⚠️ HTTP (puede requerir HTTPS)'}
          </p>
          <p>
