...
const MercadoPagoLogo: React.FC<MercadoPagoLogoProps> = ({
  ...
  if (variant === 'text') {
    return (
      <span className={cn('font-bold text-blue-600', textSizeClasses[size], className)}>
        MercadoPago
      </span>
    )
  }
  ...
  return (
    <div className={cn('flex items-center gap-2', className)}>
      ...
      <span className={cn('font-bold text-blue-600', textSizeClasses[size])}>MercadoPago</span>
    </div>
  )
}
