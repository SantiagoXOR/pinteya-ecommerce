...
export const WishlistCard = React.forwardRef<HTMLDivElement, WishlistCardProps>(
  ({
    ...
    const isOnSale = item.isOnSale || (item.originalPrice && item.discountedPrice && item.discountedPrice < item.originalPrice)
    ...
    return (
      <Card ref={ref} className={cn("relative group", className)} {...props}>
        ...
        <CardContent className="p-4">
          {/* Imagen del producto */}
          <div className="aspect-square mb-4 bg-gray-100 rounded-lg overflow-hidden cursor-pointer" onClick={handleViewDetails}>
          ...
          <div className="space-y-3">
            <div>
              <h3 className="font-semibold line-clamp-2 cursor-pointer hover:text-primary" onClick={handleViewDetails}>
                {item.title}
              </h3>
              {item.brand && (
