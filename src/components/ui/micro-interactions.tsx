...
export const FloatingAction: React.FC<FloatingActionProps> = ({
  ...
  return (
    <motion.div
      className={cn('fixed z-50 flex items-center gap-3', positions[position], className)}
...
export const InteractiveRating: React.FC<{
  ...
  size?: 'sm' | 'md' | 'lg'
  className?: string
}> = ({ value, onChange, readonly = false, size = 'md', className }) => {
        ...
        return (
          ...
          >
            <Star
              className={cn(
                sizes[size],
                isActive ? 'text-bright-sun-400 fill-current' : 'text-gray-300'
              )}
            />
          </motion.button>
        )
