...
export function RouteVisualization({
  ...
  const totalMetrics = useMemo(() => {
    const visibleRouteData = displayRoutes.filter(
      r => visibleRoutes.has(r.id) || visibleRoutes.size === 0
    )
    ...
    return {
      ...
      avgScore:
        visibleRouteData.length > 0
          ? visibleRouteData.reduce((sum, r) => sum + r.optimization_score, 0) /
            visibleRouteData.length
          : 0,
    }
      ...
      <div className='grid grid-cols-2 md:grid-cols-4 gap-4'>
        ...
        <Card>
          <CardContent className='p-4'>
            ...
            <p className='text-2xl font-bold text-purple-600'>{totalMetrics.totalShipments}</p>
          </CardContent>
        </Card>
                  ...
                  <div className='flex items-center gap-3'>
                    ...
                    <div>
                      ...
                      <p className='text-sm text-gray-600'>
                        {route.shipments.length} envíos • {route.total_distance} km •{' '}
                        {Math.round(route.estimated_time / 60)}h
                      </p>
                    </div>
                    <Badge
                      className={`ml-2 ${
