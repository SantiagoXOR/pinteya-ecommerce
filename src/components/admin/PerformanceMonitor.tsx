  ...
  if (!data) {
    return (
      <Alert>
        ...
        <AlertDescription>No hay métricas de rendimiento disponibles.</AlertDescription>
      </Alert>
    )
  }
      ...
      <div className='flex items-center justify-between'>
        <div>
          ...
          <p className='text-muted-foreground'>
            Métricas de los últimos {data.timeWindowHuman} • Actualizado:{' '}
        ...
        <Card>
          ...
          <CardContent>
            <div className='grid grid-cols-2 md:grid-cols-4 gap-4'>
              <div className='flex items-center space-x-2'>
                <Globe
                  className={`h-4 w-4 ${healthCheck.checks.api ? 'text-green-600' : 'text-red-600'}`}
                />
                <span className='text-sm'>API</span>
                <Badge
                  variant={healthCheck.checks.api ? 'default' : 'destructive'}
                  className='text-xs'
                >
              ...
              <div className='flex items-center space-x-2'>
                <Database
                  className={`h-4 w-4 ${healthCheck.checks.database ? 'text-green-600' : 'text-red-600'}`}
                />
                <span className='text-sm'>Base de Datos</span>
                <Badge
                  variant={healthCheck.checks.database ? 'default' : 'destructive'}
                  className='text-xs'
                >
              ...
              <div className='flex items-center space-x-2'>
                <Server
                  className={`h-4 w-4 ${healthCheck.checks.memory ? 'text-green-600' : 'text-red-600'}`}
                />
                <span className='text-sm'>Memoria</span>
                <Badge
                  variant={healthCheck.checks.memory ? 'default' : 'destructive'}
                  className='text-xs'
                >
              ...
              <div className='flex items-center space-x-2'>
                <Zap
                  className={`h-4 w-4 ${healthCheck.checks.performance ? 'text-green-600' : 'text-red-600'}`}
                />
                <span className='text-sm'>Rendimiento</span>
                <Badge
                  variant={healthCheck.checks.performance ? 'default' : 'destructive'}
                  className='text-xs'
                >
      ...
      <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
        <Card>
          ...
          <CardContent>
            ...
            <p className='text-xs text-muted-foreground'>En los últimos {data.timeWindowHuman}</p>
          </CardContent>
        </Card>
                ...
                <div className='space-y-2'>
                  {metrics.slowestEndpoints.map((endpoint, index) => (
                    ...
                    >
                      ...
                      <Badge variant='outline'>{formatTime(endpoint.avgTime)}</Badge>
                    </div>
                  ))}
                </div>
              ) : (
        ...
        <TabsContent value='errors' className='space-y-4'>
          <Card>
            <CardHeader>
              ...
              <CardDescription>Errores agrupados por código de estado HTTP</CardDescription>
            </CardHeader>
            <CardContent>
              {Object.keys(metrics.errorsByStatus).length > 0 ? (
                ...
                <p className='text-muted-foreground'>
                  No se registraron errores en este período. ¡Excelente!
                </p>
              )}
            </CardContent>
          </Card>
        ...
        <TabsContent value='system' className='space-y-4'>
          <Card>
            <CardHeader>
              ...
              <CardDescription>Detalles técnicos del sistema de monitoring</CardDescription>
            </CardHeader>
            <CardContent>
              <div className='grid grid-cols-2 gap-4'>
                ...
                <div>
                  ...
                  <span className='ml-2 text-sm text-muted-foreground'>{data.timeWindowHuman}</span>
                </div>
              </div>
            </CardContent>
          </Card>
