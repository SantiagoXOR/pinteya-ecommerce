  ...
  const renderHeader = () => (
    <div className='flex items-center justify-between mb-6'>
      ...
      <div className='flex items-center gap-2'>
        <Button onClick={handleRefresh} variant='outline' size='sm' disabled={state.refreshing}>
    ...
    <div className='grid grid-cols-1 md:grid-cols-3 gap-6 mb-6'>
      ...
      <Card>
        ...
        <CardContent>
          ...
          <p className='text-sm text-gray-600 mt-2'>{order.order_items?.length || 0} producto(s)</p>
        </CardContent>
      </Card>
    </div>
  )
      ...
      <CardContent>
        <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
          <div>
            ...
            <p>
              <strong>Email:</strong>{' '}
              {order.user_profiles?.email || order.payer_info?.email || 'N/A'}
            </p>
            <p>
              <strong>Teléfono:</strong>{' '}
              {order.user_profiles?.phone || order.payer_info?.phone || 'N/A'}
            </p>
          </div>
    ...
    <div className={`space-y-6 ${className}`}>
      ...
      <Tabs defaultValue='details' className='space-y-6'>
        ...
        <TabsContent value='status'>
          <OrderStatusManager order={order} onStatusChange={handleStatusChange} />
        </TabsContent>
