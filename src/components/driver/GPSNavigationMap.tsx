...
export function GPSNavigationMap({
  ...
  const [previousLocation, setPreviousLocation] = useState<{ lat: number; lng: number } | null>(
    null
  )
  ...
  const startLocationTracking = async () => {
    ...
    try {
      ...
      const { GeolocationTracker, HIGH_ACCURACY_OPTIONS, isGeolocationSupported } = await import(
  ...
  return (
    <div className='relative w-full h-full'>
      <APIProvider apiKey={process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY!}>
        <Map {...mapOptions} onLoad={handleMapLoad} className='w-full h-full'>
      ...
      <NavigationModalOverlay type='route-info' title='Información de Ruta'>
        {isNavigating && directions && (
          <RouteInfoDashboard
            routeInfo={routeInfo}
            isNavigating={isNavigating}
      ...
      <InfoModalOverlay type='real-time-tracker' title='Tracking en Tiempo Real'>
        {isNavigating && (
          <RealTimeTracker
            driverId={driverId}
            orderId={orderId}
      ...
      <FullscreenModalOverlay type='advanced-controls' title='Controles Avanzados'>
      ...
      {/* Modal de Debug GPS */}
      <InfoModalOverlay type='gps-debug' title='Debug GPS'>
        <GPSDebugInfo showDetails={true} />
      </InfoModalOverlay>

      {/* Modal de Opciones de Emergencia */}
      <InfoModalOverlay type='emergency-options' title='Opciones de Emergencia'>
        <div className='p-4 space-y-4'>
          <div className='text-center'>
            ...
            <p className='text-gray-600 mb-6'>
              ¿Necesitas recalcular la ruta o reportar un problema?
            </p>
          </div>
