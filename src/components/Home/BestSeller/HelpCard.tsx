...
const HelpCard: React.FC<HelpCardProps> = ({ categoryName, className }) => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-[calc(60%-26px)] left-5 right-20 z-20 bg-[#00703C] text-white px-4 py-3.5 rounded-[20px] font-bold text-sm leading-tight shadow-[0_4px_6px_rgba(0,0,0,0.1)] text-left">
        ¿No encontraste
        <br />
        lo que buscabas?
      </div>
      ...
      <div className="relative h-[40%] bg-[#E86C1A] flex flex-col pt-16 px-5 pb-6">
        <div className="text-left pr-20 flex-1">
          <div className="text-[#FFC805] text-2xl font-extrabold mb-1">Te ayudamos</div>
          <div className="text-white text-lg font-bold">por WhatsApp</div>
        </div>

        {/* Ícono WhatsApp en círculo blanco - Icono verde - Tamaño fijo */}
        <div className="absolute bottom-6 right-6 w-16 h-16 rounded-full bg-white flex items-center justify-center z-10 shadow-lg">
          <MessageCircle className="w-10 h-10 text-[#25D366]" strokeWidth={3} />
        </div>
      </div>
    </div>
  )
