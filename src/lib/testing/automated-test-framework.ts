...
export class AutomatedTestFramework {
    ...
    try {
      ...
      if (options?.captureScreenshot) {
        screenshot = await this.captureScreenshot(
          `step-${this.screenshotCounter++}-before`,
          description
        )
      }
      ...
      if (!screenshot) {
        screenshot = await this.captureScreenshot(
          `step-${this.screenshotCounter++}-error`,
          `Error: ${description}`
        )
      }
    }
  ...
  async finishTest(status: 'completed' | 'failed' | 'cancelled' = 'completed'): Promise<string> {
    ...
    console.log(
      `📈 Resumen: ${this.report.summary.successfulSteps}/${this.report.summary.totalSteps} pasos exitosos`
    )
  ...
  async logAction(message: string, data?: any): Promise<void> {
    ...
    await this.executeStep(`Log: ${message}`, async () => ({ logged: true, data }), {
      category: 'setup',
      severity: 'low',
    })
  }
