...
export class AdvancedTestFlowManager {
  ...
  private async notifyFlowStart(flow: TestFlow, execution: FlowExecution): Promise<void> {
    const startNotifications = flow.notifications.filter(
      n => n.triggers.includes('start') || n.triggers.includes('always')
    )
  ...
  private async notifyFlowComplete(flow: TestFlow, execution: FlowExecution): Promise<void> {
    const completeNotifications = flow.notifications.filter(
      n => n.triggers.includes(execution.status) || n.triggers.includes('always')
    )
