...
export class RealTimeMetricsStreaming {
  ...
  registerClient(
    config: StreamClientConfig,
    sendFunction: (event: StreamEvent) => Promise<boolean>
  ): StreamClient {
  ...
  private isClientEligible(client: StreamClient, event: StreamEvent): boolean {
    ...
    if (filters) {
      ...
      if (filters.tags && event.data.tags) {
        const hasMatchingTag = filters.tags.some(tag => event.data.tags.includes(tag))
        if (!hasMatchingTag) {
          return false
        }
      }
  ...
  private async sendRecentEvents(client: StreamClient): Promise<void> {
    ...
    for (const event of recentEvents) {
      try {
        ...
        logger.error(
          LogCategory.MONITORING,
          `Error sending recent event to client ${client.id}`,
          error as Error
        )
      }
    }
    ...
    while (client.eventQueue.length > 0 && this.checkRateLimit(client)) {
      ...
      if (event) {
        try {
          ...
          logger.error(
            LogCategory.MONITORING,
            `Error processing queued event for client ${client.id}`,
            error as Error
          )
        }
      }
...
export const StreamingUtils = {
  ...
  createCriticalAlertsClientConfig(clientId: string): StreamClientConfig {
    return {
      clientId,
      subscriptions: [StreamEventType.ALERT_CREATED, StreamEventType.BUDGET_VIOLATION],
      filters: {
