      ...
      try {
        ...
        if (!rateLimitResult.allowed) {
          ...
          if (options.enableLogging !== false) {
            console.warn(
              `[RATE_LIMIT_MIDDLEWARE] Límite excedido para ${request.nextUrl.pathname}`,
              {
          ...
          const response = NextResponse.json(
            {
              error:
                ENTERPRISE_RATE_LIMIT_CONFIGS[options.configName].message || 'Rate limit exceeded',
  ...
  try {
    ...
    if (enterpriseContext && !finalConfigName) {
      ...
      if (enterpriseContext.securityLevel === 'critical') {
        customConfig.maxRequests = Math.floor(
          (ENTERPRISE_RATE_LIMIT_CONFIGS[finalConfigName].maxRequests || 50) * 0.5
        )
      } else if (enterpriseContext.securityLevel === 'high') {
        customConfig.maxRequests = Math.floor(
          (ENTERPRISE_RATE_LIMIT_CONFIGS[finalConfigName].maxRequests || 50) * 0.75
        )
      }
 ...
 */
export const withAdminRateLimit = () => withEnterpriseRateLimit({ configName: 'ADMIN_API' })

/**
 * Rate limiting para APIs de pagos
 */
export const withPaymentRateLimit = () => withEnterpriseRateLimit({ configName: 'PAYMENT_API' })

/**
 * Rate limiting para APIs públicas
 */
export const withPublicRateLimit = () => withEnterpriseRateLimit({ configName: 'PUBLIC_API' })

/**
 * Rate limiting para webhooks
 */
export const withWebhookRateLimit = () => withEnterpriseRateLimit({ configName: 'WEBHOOK_API' })
