...
export class CircuitBreaker {
  ...
  constructor(
    ...
    logger.debug(
      LogLevel.DEBUG,
      `Circuit Breaker initialized: ${name}`,
      {
        config: this.config,
      },
      LogCategory.SYSTEM
  ...
  private async onFailure(error: Error): Promise<void> {
    ...
    logger.warn(
      LogLevel.WARN,
      `Circuit Breaker failure: ${this.name}`,
      {
        error: error.message,
  ...
  private transitionToHalfOpen(): void {
    ...
    logger.info(
      LogLevel.INFO,
      `Circuit Breaker HALF-OPEN: ${this.name}`,
      {
  ...
  private async recordMetrics(
    ...
    try {
      ...
      // Métricas de estado
      await metricsCollector.recordMetric(
        `circuit_breaker.${this.name}.state_changes`,
        this.stateChanges
      )
      await metricsCollector.recordMetric(
      ...
      logger.error(
        LogLevel.ERROR,
        `Failed to record circuit breaker metrics: ${this.name}`,
        {
  ...
  reset(): void {
    ...
    logger.info(
      LogLevel.INFO,
      `Circuit Breaker manually reset: ${this.name}`,
      {},
      LogCategory.SYSTEM
    )
  }
