...
export async function createPaymentPreference(data: CreatePreferenceData) {
  ...
  const circuitResult = await executeMercadoPagoCritical(async () => {
      ...
      async () => {
        ...
        const preferenceData = {
          ...
          ...(isSandbox && {
            ...
            notification_url:
              data.notification_url || `${baseUrl}/api/payments/webhook?sandbox=true`,
          }),
        }
    ...
    if (retryResult.success) {
      ...
      logger.error(
        LogCategory.PAYMENT,
        'Failed to create payment preference after retries',
        retryResult.error!
      )
      throw new Error(
        retryResult.error?.message || 'Error creating payment preference after retries'
      )
    }
  })
...
export function validateWebhookOrigin(request: Request): boolean {
  ...
  console.log(
    '[WEBHOOK] MODO PERMISIVO ACTIVADO - Permitiendo todas las solicitudes para diagnosticar'
  )
  return true
