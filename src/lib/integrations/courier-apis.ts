...
export class OCAApi extends CourierAPI {
  ...
  private parseOCATracking(response: any, trackingNumber: string): TrackingResponse {
    ...
    if (response.Eventos) {
      events.push(
        ...response.Eventos.map((evento: any) => ({
...
export class CourierAPIFactory {
  ...
  static async getMultipleQuotes(
    ...
    return results
      .filter(
        (result): result is PromiseFulfilledResult<ShippingQuote[]> => result.status === 'fulfilled'
      )
  ...
  static async trackMultipleShipments(
    ...
    return results
      .filter(
        (result): result is PromiseFulfilledResult<TrackingResponse | null> =>
