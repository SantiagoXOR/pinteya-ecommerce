...
export class CacheAnalyticsManager {
   ...
   */
  getHistoricalMetrics(key: string, startTime: number, endTime: number): CachePerformanceMetrics[] {
    const keyMetrics = this.metrics.get(key) || []
    return keyMetrics.filter(metric => metric.timestamp >= startTime && metric.timestamp <= endTime)
  }
  ...
  async analyzeTrends(period: 'hour' | 'day' | 'week' | 'month'): Promise<TrendAnalysis> {
    ...
    this.metrics.forEach(metrics => {
      const periodMetrics = metrics.filter(m => m.timestamp >= startTime && m.timestamp <= now)
    ...
    const responseTrend = this.calculateTrend(
      firstHalfAvg.avgResponseTime,
      secondHalfAvg.avgResponseTime
    )
   ...
   */
  async generateReport(startTime: number, endTime: number): Promise<CacheReport> {
  ...
  async cleanupOldMetrics(maxAge: number = 7 * 24 * 60 * 60 * 1000): Promise<void> {
    ...
    logger.info(
      LogCategory.CACHE,
      `Limpieza completada: ${oldAlerts.length} alertas antiguas eliminadas`
    )
  }
  ...
  private getTopKeys(limit: number): Array<{ key: string; hits: number; misses: number }> {
    ...
    return keyStats.sort((a, b) => b.hits + b.misses - (a.hits + a.misses)).slice(0, limit)
  }
...
export const CacheAnalyticsUtils = {
  ...
  getQuickSummary(): {
    ...
    const avgHitRate =
      Object.values(currentMetrics).reduce((sum, m) => sum + m.hitRate, 0) / totalKeys || 0
