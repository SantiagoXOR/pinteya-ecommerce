...
export class CacheWarmingSystem {
  ...
  async executeTask(taskId: string): Promise<WarmupResult> {
    ...
    try {
      ...
      task.avgDuration =
        (task.avgDuration * (task.successCount - 1) + result.duration) / task.successCount
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        if (attempt < maxRetries - 1) {
          ...
          logger.warn(
            LogCategory.CACHE,
            `Reintentando tarea ${task.name} en ${delay}ms (intento ${attempt + 1}/${maxRetries})`
          )
  ...
  async executeStrategy(strategy: WarmupStrategy): Promise<WarmupResult[]> {
    const strategyTasks = Array.from(this.tasks.values()).filter(
      task => task.config.strategy === strategy
    )

    const results = await Promise.allSettled(strategyTasks.map(task => this.executeTask(task.id)))
      ...
      tasks.push(
        {
          ...
          fetcher: () => cacheWarmingSystem['fetchPopularProducts'](),
          strategy: 'PRODUCT_DATA',
        },
        {
          key: 'home:main-categories',
          fetcher: () => cacheWarmingSystem['fetchMainCategories'](),
          strategy: 'PRODUCT_DATA',
        }
      )
