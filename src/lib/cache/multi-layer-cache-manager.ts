...
export class MultiLayerCacheManager {
  ...
  async invalidatePattern(pattern: string, layers?: CacheLayer[]): Promise<void> {
    ...
    const invalidatePromises = targetLayers.map(async layer => {
      try {
        ...
        logger.error(
          LogCategory.CACHE,
          `Error invalidando patrón en capa ${layer}:`,
          error as Error
        )
      }
    })
  ...
  private async propagateToHigherPriorityLayers<T>(
    ...
    const higherPriorityLayers = this.getEnabledLayersByPriority().filter(
      layer => LAYER_CONFIGS[layer].priority < currentPriority
    )
  ...
  private recordMetric(layer: CacheLayer, type: 'hit' | 'miss', latency: number): void {
    ...
    metrics.avgLatency =
      (metrics.avgLatency * (metrics.totalRequests - 1) + latency) / metrics.totalRequests
