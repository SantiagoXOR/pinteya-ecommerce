 ...
 */
function convertGeolocationError(error: GeolocationPositionError, context?: string): GeolocationError {
  ...
  switch (error.code) {
    case error.PERMISSION_DENIED:
      ...
      message = 'Permisos de ubicación denegados por el usuario. Habilita la ubicación en la configuración del navegador.';
      retryable = false;
      break;
...
export async function getCurrentPosition(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      const currentOptions = attempt === 0 ? options :
                           attempt === 1 ? FALLBACK_OPTIONS :
...
export class GeolocationTracker {
  ...
  async start(): Promise<void> {
    ...
    if (permission === 'denied') {
      const error: GeolocationError = {
        code: 1,
        message: 'Permisos de geolocalización denegados. Habilita la ubicación en la configuración del navegador.',
        type: 'PERMISSION_DENIED',
        timestamp: Date.now(),
        retryable: false
      };
  ...
  private handlePositionError(error: GeolocationError): void {
    ...
    if (this.retryCount < this.maxRetries) {
      ...
      console.log(`Reintentando geolocalización en ${retryDelay}ms (intento ${this.retryCount}/${this.maxRetries})`);
