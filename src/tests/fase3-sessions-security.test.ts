...
describe('Fase 3: Sesiones y Seguridad', () => {
  ...
  describe('Tipos de Datos', () => {
    ...
    it('debería tener interfaces correctas para UserActivity', () => {
      ...
      expect(['auth', 'profile', 'order', 'security', 'session', 'preference']).toContain(mockActivity.category);
    });
  });
  ...
  describe('Migración de Base de Datos', () => {
    it('debería tener migración SQL definida', () => {
      const migrationPath = require.resolve('@/../supabase/migrations/20250913_user_sessions_and_activity.sql');
...
export function validateAnomalyStructure(anomaly: any): boolean {
  const requiredProps = ['isAnomalous', 'severity', 'type', 'description', 'metadata', 'recommendations'];
