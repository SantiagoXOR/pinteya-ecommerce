...
import { LogisticsDashboardResponse, UseLogisticsDashboardReturn } from '@/types/logistics'
...
export function useLogisticsDashboard(options?: {
  ...
  const updateCache = useCallback(
    (
      updater: (
        oldData: LogisticsDashboardResponse['data'] | undefined
      ) => LogisticsDashboardResponse['data']
    ) => {
  ...
  const derivedMetrics = data
    ? {
        ...
        active_shipments_rate:
          data.stats.total_shipments > 0
            ? ((data.stats.in_transit_shipments + data.stats.pending_shipments) /
                data.stats.total_shipments) *
              100
            : 0,
        ...
        shipments_trend:
          data.performance_metrics.length >= 14
            ? (() => {
                ...
                const previous7Total = previous7Days.reduce(
                  (acc, day) => acc + day.shipments_count,
                  0
                )
        ...
        best_performing_courier:
          data.carrier_performance.length > 0
...
export function useCarrierPerformance() {
  ...
  const sortedCarriers =
    data?.carrier_performance.sort((a, b) => b.on_time_rate - a.on_time_rate) || []
  ...
  return {
    ...
    averageOnTimeRate:
      sortedCarriers.length > 0
        ? sortedCarriers.reduce((acc, carrier) => acc + carrier.on_time_rate, 0) /
