...
export function useOrdersCache(options: CacheOptions): UseOrdersCacheReturn {
  ...
  const getCachedData = useCallback(
    (filters: StrictOrderFilters): StrictOrdersListResponse | null => {
  ...
  const setCachedData = useCallback(
    (filters: StrictOrderFilters, data: StrictOrdersListResponse): void => {
      if (!options.enableCache) {
        return
      }
      ...
      const now = Date.now()
      if (
        now - lastCleanupRef.current >
        (options.cleanupInterval || CACHE_CONFIG.CLEANUP_INTERVAL)
      ) {
  ...
  const isRequestTooRecent = useCallback(
    (filters: StrictOrderFilters): boolean => {
      const key = getCacheKey(filters)
      ...
      return (
        timeSinceLastRequest < (options.minRequestInterval || CACHE_CONFIG.MIN_REQUEST_INTERVAL)
      )
    },
  ...
  const setPendingRequest = useCallback(
    (filters: StrictOrderFilters, promise: Promise<any>): void => {
      const key = getCacheKey(filters)
