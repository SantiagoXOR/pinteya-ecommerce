...
export function useOrdersState(
  ...
  const updateOrder = useCallback(
    (orderId: string, updates: Partial<StrictOrderEnterprise>): void => {
      setState(prev => ({
        ...prev,
        orders: prev.orders.map(order => (order.id === orderId ? { ...order, ...updates } : order)),
        lastUpdated: new Date(),
      }))
    },
    []
  ...
  const hasFiltersChanged = useCallback(
    (newFilters: StrictOrderFilters): boolean => {
