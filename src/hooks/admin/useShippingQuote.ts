...
export function useShippingQuote() {
  ...
  const getQuote = useCallback(
    async (request: ShippingQuoteRequest) => {
...
export function useQuoteComparison(quotes: ShippingQuote[]) {
  const comparison = {
    cheapest: quotes.reduce((prev, current) => (prev.cost < current.cost ? prev : current)),
    ...
    averageDeliveryTime:
      quotes.reduce((acc, quote) => acc + quote.estimated_delivery_days, 0) / quotes.length,
    priceRange: {
...
export function useQuoteCalculator() {
  ...
  const calculateEstimate = useCallback(
    (courier: Courier) => {
      if (!calculatorData.weight || !courier) {
        return null
      }
