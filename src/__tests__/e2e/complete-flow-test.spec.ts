...
test.describe('Complete Flow Integration Test', () => {
    ...
    try {
      ...
      await testFlowManager.executeStep(flowId, {
        ...
        action: async () => {
          ...
          const screenshotId = await screenshotManager.captureStep(
            flowId,
            'homepage-load',
            page,
            { stepName: 'Homepage Loaded', status: 'success' }
          );
      ...
      await testFlowManager.executeStep(flowId, {
        ...
        action: async () => {
          ...
          const screenshotId = await screenshotManager.captureStep(
            ...
            'products-page',
            page,
            { stepName: 'Products Page', status: 'success' }
          );
      ...
      await testFlowManager.executeStep(flowId, {
        ...
        action: async () => {
          ...
          const screenshotId = await screenshotManager.captureStep(
            ...
            'product-detail',
            page,
            { stepName: 'Product Detail View', status: 'success' }
          );
      ...
      await testFlowManager.executeStep(flowId, {
        ...
        action: async () => {
          ...
          const screenshotId = await screenshotManager.captureStep(
            ...
            'add-to-cart',
            page,
            { stepName: 'Product Added to Cart', status: 'success' }
          );
      ...
      await testFlowManager.executeStep(flowId, {
        ...
        action: async () => {
          ...
          const screenshotId = await screenshotManager.captureStep(
            ...
            'cart-view',
            page,
            { stepName: 'Shopping Cart View', status: 'success' }
          );
      ...
      await testFlowManager.executeStep(flowId, {
        ...
        action: async () => {
          ...
          const screenshotId = await screenshotManager.captureStep(
            ...
            'checkout-form',
            page,
            { stepName: 'Checkout Form', status: 'success' }
          );
      ...
      const errorScreenshotId = await screenshotManager.captureStep(
        flowId,
        'error-state',
        page,
        { stepName: 'Test Failure', status: 'failure' }
      );
  ...
  test('Verify Test Results in Dashboard', async () => {
    ...
    await screenshotManager.captureStep(
      'dashboard-verification',
      'dashboard-final',
      page,
      { stepName: 'Dashboard with Test Results', status: 'success' }
    );
  });
  ...
  test('API Integration Test', async () => {
    ...
    const screenshotsResponse = await page.request.get(`/api/admin/test-screenshots?action=list&executionId=${executionId}`);
