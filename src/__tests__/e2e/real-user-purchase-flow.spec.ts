...
test.describe('Real User Purchase Flow - Pinteya E-commerce', () => {
  ...
  test('Complete Purchase Flow - From Homepage to Order Confirmation', async () => {
    ...
    const productLinks = page.locator(
      'a[href*="/products"], a[href*="/categoria"], [data-testid*="product"]'
    )
    ...
    if (!cartButtonFound) {
      ...
      const buyButtons = page.locator(
        'button:has-text("Comprar"), button:has-text("Buy"), button[class*="buy"]'
      )
  ...
  test('Complete Checkout Form and Payment Process', async () => {
    ...
    const formFields = [
      ...
      {
        selector: 'input[name="postalCode"], input[name="postal_code"], #postalCode',
