...
async function measureSkeletonDuration(
  page: Page,
  ...
  test('debe mostrar skeletons estructuralmente similares al contenido final', async ({ page }) => {
    ...
    const initialStructure = await page.evaluate(() => {
      ...
      const skeletons = Array.from(
        document.querySelectorAll('[data-testid*="skeleton"], .animate-pulse')
      )
...
test.describe('Skeleton Validation - Productos', () => {
  test('debe mostrar skeleton de lista de productos', async ({ page }) => {
    ...
    const productSkeletons = await page
      .locator(
  ...
  test('debe mostrar skeleton de formulario de producto', async ({ page }) => {
    ...
    const formSkeletons = await page
      .locator('[data-testid*="form-skeleton"], .form-skeleton, form [class*="skeleton"]')
      .count()
...
test.describe('Skeleton Validation - Logística', () => {
  test('debe mostrar skeleton de dashboard de logística', async ({ page }) => {
    ...
    const logisticsSkeletons = await page
      .locator(
    ...
    const logisticsContent = await page
      .locator(
  ...
  test('debe manejar skeleton de mapa correctamente', async ({ page }) => {
    ...
    const mapContainers = await page
      .locator('[data-testid*="map"], [class*="map"], #map, .logistics-map')
      .count()
    ...
    if (mapContainers > 0) {
      ...
      const mapDimensions = await page
        .locator('[data-testid*="map"], [class*="map"]')
        .first()
        .evaluate(el => {
...
test.describe('Skeleton Performance', () => {
  test('debe mostrar skeletons rápidamente', async ({ page }) => {
    ...
    const skeletonAppearTime = await page
      .waitForSelector('[data-testid*="skeleton"], .animate-pulse, .skeleton', { timeout: 3000 })
