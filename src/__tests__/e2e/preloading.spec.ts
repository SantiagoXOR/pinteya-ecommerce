...
function wasResourcePreloaded(
  requests: Array<{ url: string; timestamp: number; type: string }>,
...
function getPreloadToUsageTime(
  requests: Array<{ url: string; timestamp: number; type: string }>,
  resourcePattern: string
) {
...
test.describe('Preloading Functionality', () => {
  test('debe precargar componentes admin después del delay configurado', async ({ page }) => {
    ...
    const adminRequests = requests.filter(
      req => req.url.includes('admin') || req.url.includes('chunk') || req.url.includes('_next')
    )
  ...
  test('debe precargar componentes de productos inteligentemente', async ({ page }) => {
    ...
    const productRequests = requests.filter(
      req =>
        req.url.includes('product') || req.url.includes('inventory') || req.url.includes('catalog')
    )
  ...
  test('debe precargar componentes de logística bajo demanda', async ({ page }) => {
    ...
    const logisticsLink = page
      .locator('a[href*="/admin/logistics"], a:has-text("Logística")')
      .first()
    ...
    if ((await logisticsLink.count()) > 0) {
      ...
      const logisticsRequests = requests.filter(
        req =>
          req.url.includes('logistics') || req.url.includes('map') || req.url.includes('tracking')
      )
...
test.describe('Preloading Hooks Integration', () => {
  ...
  test('debe manejar preloading condicional basado en user behavior', async ({ page }) => {
    ...
    const productsSection = page
      .locator(':has-text("Productos"), [data-section="products"]')
      .first()
    ...
    if ((await productsSection.count()) > 0) {
      ...
      const productPreloadRequests = requests.filter(
        req => req.url.includes('product') && req.timestamp > Date.now() - 2000
      )
    ...
    if ((await ordersSection.count()) > 0) {
      ...
      const orderPreloadRequests = requests.filter(
        req => req.url.includes('order') && req.timestamp > Date.now() - 2000
      )
...
test.describe('Preloading Performance', () => {
  ...
  test('debe gestionar memoria eficientemente durante preloading', async ({ page }) => {
    ...
    if (initialMemory && afterPreloadMemory) {
      ...
      console.log(
        `Memory increase after preloading: ${memoryIncrease} bytes (${memoryIncreasePercent.toFixed(2)}%)`
      )
  ...
  test('debe cancelar preloading cuando no es necesario', async ({ page }) => {
    ...
    const duplicateRequests = recentRequests.filter(
