...
const mockDashboardData = {
  ...
  recentOrders: [
    {
      ...
      items: [
        { product_name: 'Laptop Gaming', quantity: 1, price: 299.99 }
      ]
    },
    {
      id: 'order-2',
      ...
      items: [
        { product_name: 'Mouse Inalámbrico', quantity: 2, price: 74.75 }
      ]
    }
  ],
  salesData: {
  ...
  topProducts: [
    ...
    { id: 2, name: 'Mouse Inalámbrico', sales: 45, revenue: 3367.50 },
    { id: 3, name: 'Teclado Mecánico', sales: 32, revenue: 4800.00 }
  ],
  lowStockProducts: [
        ...
        const responses = {
          orders: {
            select: jest.fn(() => ({
              count: jest.fn().mockResolvedValue({ count: mockDashboardData.stats.totalOrders }),
              eq: jest.fn(() => ({
                count: jest.fn().mockResolvedValue({ count: mockDashboardData.stats.pendingOrders })
              }))
            }))
          },
          products: {
            select: jest.fn(() => ({
              count: jest.fn().mockResolvedValue({ count: mockDashboardData.stats.totalProducts }),
              lt: jest.fn(() => ({
                count: jest.fn().mockResolvedValue({ count: mockDashboardData.stats.lowStockProducts })
              }))
            }))
          },
          users: {
