...
function detectPerformanceIssues(metrics: APIMetrics) {
  ...
  if (responseTime > 5000) {
    // 5 segundos
    productionMonitor.trackError(new Error(`Slow API response: ${responseTime}ms`), {
  ...
  if (statusCode >= 500) {
    const recentErrors = Array.from(metricsCache.values()).filter(
      m =>
        m.path === path &&
        m.method === method &&
        m.statusCode >= 500 &&
    ...
    if (recentErrors.length >= 5) {
      productionMonitor.trackError(new Error(`High error rate detected on ${method} ${path}`), {
...
export class APIAnalytics {
  ...
  static getAggregatedMetrics(timeWindow: number = 3600000): {
    ...
    const now = Date.now()
    const recentMetrics = Array.from(metricsCache.values()).filter(
...
${Object.entries(metrics.errorsByStatus)
