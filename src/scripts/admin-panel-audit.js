    ...
    try {
      ...
      if (response.includes('/admin/products')) {
        ...
        this.addTestResult(
          ...
          'route_accessible',
          false,
          'Ruta /admin/products no accesible'
        )
      }
      ...
      const tests = [
        ...
        {
          selector: '[data-testid="products-table"], table',
          name: 'products_table',
          description: 'Tabla de productos presente',
        },
        {
          selector: 'button:has-text("Agregar"), button:has-text("Nuevo")',
          name: 'add_button',
          description: 'Botón agregar producto presente',
        },
        {
          selector: 'input[type="search"], input[placeholder*="buscar"]',
          name: 'search_input',
          description: 'Campo de búsqueda presente',
        },
        {
          selector: 'select, [role="combobox"]',
          name: 'filters',
          description: 'Filtros presentes',
        },
      ]
      ...
      this.addTestResult(
        ...
        'general_error',
        false,
        `Error general en Fase 1: ${error.message}`
      )
    }
  }
    ...
    try {
      ...
      if (this.page.url().includes('/admin/orders')) {
        ...
        this.addTestResult(
          ...
          'route_accessible',
          false,
          'Ruta /admin/orders no accesible'
        )
      }
      ...
      const orderTests = [
        ...
        {
          selector: '[data-testid="orders-table"], table',
          name: 'orders_table',
          description: 'Tabla de órdenes',
        },
        {
          selector: '[data-testid="order-status"], .status',
          name: 'order_status',
          description: 'Estados de órdenes visibles',
        },
        {
          selector: 'button:has-text("Bulk"), button:has-text("Masivo")',
          name: 'bulk_operations',
          description: 'Operaciones masivas',
        },
        {
          selector: '[data-testid="analytics"], .analytics',
          name: 'analytics_panel',
          description: 'Panel de analytics',
        },
        {
          selector: 'select[data-testid="status-filter"]',
          name: 'status_filters',
          description: 'Filtros de estado',
        },
      ]
      ...
      for (const state of expectedStates) {
        try {
          const stateElement = await this.page
            .locator(`text=${state}, [data-status="${state}"]`)
            .first()
      ...
      this.addTestResult(
        ...
        'order_states',
        statesFound >= 4,
        `Estados encontrados: ${statesFound}/8`
      )
    } catch (error) {
      this.addTestResult(
        'phase2_orders',
        'general_error',
        false,
        `Error general en Fase 2: ${error.message}`
      )
    }
  }
  ...
  async auditPhase3Monitoring() {
    ...
    try {
      ...
      this.addTestResult(
        ...
        'no_401_error',
        !hasError401,
        hasError401 ? 'Error 401 detectado' : 'Sin errores 401'
      )
      ...
      if (this.page.url().includes('/admin/monitoring')) {
        this.addTestResult(
          ...
          'route_accessible',
          true,
          'Ruta /admin/monitoring accesible'
        )
      } else {
        this.addTestResult(
          'phase3_monitoring',
          'route_accessible',
          false,
          'Ruta /admin/monitoring no accesible'
        )
      }
      ...
      const monitoringTests = [
        ...
        {
          selector: '[data-testid="metrics-dashboard"], .metrics',
          name: 'metrics_dashboard',
          description: 'Dashboard de métricas',
        },
        {
          selector: '[data-testid="circuit-breaker"], .circuit-breaker',
          name: 'circuit_breakers',
          description: 'Circuit breakers',
        },
        {
          selector: '[data-testid="audit-trail"], .audit',
          name: 'audit_trail',
          description: 'Audit trail',
        },
        {
          selector: '[data-testid="alerts"], .alerts',
          name: 'alerts_panel',
          description: 'Panel de alertas',
        },
        {
          selector: '[data-testid="health-checks"], .health',
          name: 'health_checks',
          description: 'Health checks',
        },
        {
          selector: '[data-testid="real-time"], .real-time',
          name: 'real_time_data',
      ...
      try {
        const response = await this.page.request.get(
          `${AUDIT_CONFIG.baseUrl}/api/admin/monitoring/metrics`
        )
        this.addTestResult(
          'phase3_monitoring',
          'metrics_api',
          response.ok(),
          `API metrics status: ${response.status()}`
        )
      } catch (error) {
        this.addTestResult(
          'phase3_monitoring',
          'metrics_api',
          false,
          `Error API metrics: ${error.message}`
        )
      }
    } catch (error) {
      this.addTestResult(
        ...
        'general_error',
        false,
        `Error general en Fase 3: ${error.message}`
      )
    }
  }
...
${Object.entries(this.auditResults.phases)
  .map(
    ([phaseName, phase]) => `
### ${phaseName}
