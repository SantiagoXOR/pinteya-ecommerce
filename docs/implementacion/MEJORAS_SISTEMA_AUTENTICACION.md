...
const ROLE_PERMISSIONS: Record<Role, Permission[]> = {
  ...
  [Role.SUPPORT]: [Permission.ORDERS_READ, Permission.USERS_READ],
  [Role.CUSTOMER]: [],
}
...
export class SessionValidator {
  ...
  static async validateSessionContext(
    req: NextRequest,
    userId: string
  ...
  private static detectAbnormalRequestRate(activity: any[]): boolean {
    const recentRequests = activity.filter(a => Date.now() - a.timestamp < 5 * 60 * 1000) // Últimos 5 minutos
...
export function withSecurityHeaders() {
  return (req: NextRequest) => {
    ...
    response.headers.set(
      'Permissions-Policy',
      ['camera=()', 'microphone=()', 'geolocation=()', 'payment=(self)', 'usb=()'].join(', ')
    )
...
export class AccountRecovery {
  ...
  static async completePasswordReset(token: string, newPassword: string): Promise<boolean> {
    try {
