...
test.describe('Enhanced Header - Pinteya E-commerce', () => {
  ...
  test.describe('Performance', () => {
    test('should load quickly', async ({ page }) => {
      ...
      // Verificar que carga en menos de 10 segundos (más permisivo)
  ...
  test.describe('Error Handling', () => {
    test('should handle missing components gracefully', async ({ page }) => {
      // Verificar que no hay errores en consola relacionados con dropdowns
      const errors: string[] = []
      page.on('console', msg => {
        if (msg.type() === 'error') {
      ...
      const dropdownErrors = errors.filter(
        error => error.includes('dropdown') || error.includes('radix')
      )
